...
static int test_bounded_residency(void)
{
    ...
    {
        uint64_t sample_bytes = (uint64_t)layout.samples_per_region * sizeof(henka_terrain_sample);
        henka_terrain_world_stats before_release;
        if (henka_terrain_world_get_stats(world, &before_release) != HENKA_SUCCESS ||
            before_release.cpu_bytes < sample_bytes ||
            henka_terrain_world_release_chunk(world, chunk) != HENKA_SUCCESS ||
            henka_terrain_world_release_region(world, region) != HENKA_SUCCESS ||
            henka_terrain_world_get_stats(world, &stats) != HENKA_SUCCESS ||
            stats.cpu_bytes + sample_bytes != before_release.cpu_bytes)
        {
            henka_terrain_world_destroy(world);
            return 0;
        }
...
static int test_regeneration_queue(void)
{
    ...
    HENKA_TEST_ASSERT(henka_terrain_world_regeneration_request_is_current(
        world, &request));
    if (henka_terrain_world_set_region_revision(
            world, second, 9U, 10U, true) != HENKA_SUCCESS ||
        henka_terrain_world_regeneration_request_is_current(
            world, &request) ||
    ...
    if (henka_terrain_world_regeneration_request_is_current(
            world, &request) ||
        henka_terrain_world_regeneration_request_is_current(
            NULL, &request) ||
        henka_terrain_world_request_regeneration(
    ...
    if (henka_terrain_world_request_regeneration(
            ...
            HENKA_TERRAIN_REGENERATION_PHYSICS |
            HENKA_TERRAIN_REGENERATION_RENDER) != HENKA_SUCCESS ||
        henka_terrain_world_cancel_regeneration(
            world, first, HENKA_TERRAIN_REGENERATION_PHYSICS) != HENKA_SUCCESS ||
        henka_terrain_world_pop_regeneration(world, &request) != HENKA_SUCCESS ||
        request.targets != HENKA_TERRAIN_REGENERATION_RENDER ||
        henka_terrain_world_request_regeneration(
        ...
        henka_terrain_world_cancel_regeneration(
            world, first, HENKA_TERRAIN_REGENERATION_RENDER) != HENKA_SUCCESS ||
        henka_terrain_world_cancel_regeneration(
            world, first, HENKA_TERRAIN_REGENERATION_RENDER) ==
            HENKA_SUCCESS ||
        henka_terrain_world_get_stats(world, &stats) != HENKA_SUCCESS ||
