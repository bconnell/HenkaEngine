...
static int test_stale_nonresident_request_is_cancelled_before_rebuild(void)
{
    ...
    engine_config.application_name = "Terrain Stale Request Test";
    ...
    passed = after.failed_rebuilds == before.failed_rebuilds &&
        after.resident_chunks == 0U &&
        henka_scene_get_entity_count(scene) == 0U &&
        henka_terrain_render_runtime_get_chunk(
            runtime, (henka_terrain_chunk_id){0, 0}, &chunk_info) == HENKA_ERROR_INVALID_ARGUMENT;
