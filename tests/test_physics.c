...
static void henka_test_physics_direct_state_setters(void)
{
    henka_physics_world* world;
    ...
    henka_physics_material invalid_material;
    henka_vec3 invalid_velocity;
    ...
    material = henka_physics_material_default();
    material.restitution = 0.8f;
    material.static_friction = 0.9f;
    material.dynamic_friction = 0.7f;
    material.linear_damping = 0.2f;
    material.angular_damping = 0.3f;
    HENKA_TEST_ASSERT(henka_physics_body_set_material(world, body, material) == HENKA_SUCCESS);
    HENKA_TEST_ASSERT(henka_physics_body_get_state(world, body, &before) == HENKA_SUCCESS);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(before.material.restitution, 0.8f, 0.0001f);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(before.material.static_friction, 0.9f, 0.0001f);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(before.material.dynamic_friction, 0.7f, 0.0001f);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(before.material.linear_damping, 0.2f, 0.0001f);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(before.material.angular_damping, 0.3f, 0.0001f);

    invalid_material = material;
    invalid_material.restitution = 1.01f;
    HENKA_TEST_ASSERT(henka_physics_body_set_material(
        world,
        body,
        invalid_material) == HENKA_ERROR_INVALID_ARGUMENT);
    ...
    HENKA_TEST_ASSERT(henka_physics_body_set_material(
        ...
        material) == HENKA_ERROR_INVALID_ARGUMENT);

    HENKA_TEST_ASSERT(henka_physics_body_set_angular_velocity(
        world,
        body,
        (henka_vec3){1.0f, -2.0f, 3.0f}) == HENKA_SUCCESS);
    HENKA_TEST_ASSERT(henka_physics_body_get_state(world, body, &before) == HENKA_SUCCESS);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(before.angular_velocity.x, 1.0f, 0.0001f);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(before.angular_velocity.y, -2.0f, 0.0001f);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(before.angular_velocity.z, 3.0f, 0.0001f);

    invalid_velocity = (henka_vec3){NAN, 0.0f, 0.0f};
    HENKA_TEST_ASSERT(henka_physics_body_set_angular_velocity(
        world,
        body,
        invalid_velocity) == HENKA_ERROR_INVALID_ARGUMENT);
    HENKA_TEST_ASSERT(henka_physics_body_get_state(world, body, &after) == HENKA_SUCCESS);
    HENKA_TEST_ASSERT(memcmp(&before, &after, sizeof(before)) == 0);
    HENKA_TEST_ASSERT(henka_physics_body_set_angular_velocity(
        world,
        HENKA_INVALID_PHYSICS_BODY_ID,
        (henka_vec3){0.0f, 0.0f, 0.0f}) == HENKA_ERROR_INVALID_ARGUMENT);

    henka_physics_world_destroy(world);
}

static void henka_test_physics_world_configuration(void)
{
    henka_physics_world* world;
    henka_physics_body_desc desc;
    henka_physics_body_id body;
    ...
    HENKA_TEST_ASSERT(henka_physics_world_set_gravity(
        world,
        (henka_vec3){1.0f, -2.0f, 3.0f}) == HENKA_SUCCESS);
    gravity = henka_physics_world_get_gravity(world);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(gravity.x, 1.0f, 0.0001f);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(gravity.y, -2.0f, 0.0001f);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(gravity.z, 3.0f, 0.0001f);
    HENKA_TEST_ASSERT(henka_physics_world_set_gravity(
        world,
        (henka_vec3){NAN, 0.0f, 0.0f}) == HENKA_ERROR_INVALID_ARGUMENT);
    gravity = henka_physics_world_get_gravity(world);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(gravity.x, 1.0f, 0.0001f);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(gravity.y, -2.0f, 0.0001f);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(gravity.z, 3.0f, 0.0001f);
    ...
    desc.linear_velocity.x = 1.0f;
    HENKA_TEST_ASSERT(henka_physics_body_create(world, &desc, &body) == HENKA_SUCCESS);
    HENKA_TEST_ASSERT(henka_physics_world_step(world, 0.1f) == HENKA_SUCCESS);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(
        henka_physics_test_get_accumulator(world),
        0.1f,
        0.000001f);
    HENKA_TEST_ASSERT(henka_physics_body_get_state(world, body, &before) == HENKA_SUCCESS);
    fixed_timestep = henka_physics_world_get_fixed_timestep(world);
    HENKA_TEST_ASSERT(henka_physics_world_set_fixed_timestep(
        world,
        0.0f) == HENKA_ERROR_INVALID_ARGUMENT);
    HENKA_TEST_ASSERT(henka_physics_world_set_fixed_timestep(
        world,
        NAN) == HENKA_ERROR_INVALID_ARGUMENT);
    HENKA_TEST_ASSERT(henka_physics_world_set_fixed_timestep(
        world,
        1.0001f) == HENKA_ERROR_INVALID_ARGUMENT);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(
    ...
    HENKA_TEST_ASSERT_FLOAT_CLOSE(
        ...
        0.0f,
        0.000001f);
    HENKA_TEST_ASSERT(henka_physics_world_get_gravity(NULL).x == 0.0f);
    HENKA_TEST_ASSERT(henka_physics_world_get_fixed_timestep(NULL) == 0.0f);
    henka_physics_world_destroy(world);
}
...
static void henka_test_physics_triangle_mesh_shape_pairs(void)
{
    const henka_vec3 vertices[4] = {
        {-2.0f, 0.0f, -2.0f},
        {2.0f, 0.0f, -2.0f},
    ...
    mesh_collider = henka_test_triangle_mesh_collider(
        vertices,
        4U,
        indices,
        6U);
    ...
    world = NULL;
    shape_body = HENKA_INVALID_PHYSICS_BODY_ID;
    mesh_body = HENKA_INVALID_PHYSICS_BODY_ID;
    HENKA_TEST_ASSERT(henka_physics_world_create(&world) == HENKA_SUCCESS);
    desc = henka_test_physics_body(
        HENKA_PHYSICS_BODY_DYNAMIC,
        henka_physics_collider_capsule(0.5f, 0.5f),
    ...
    desc = henka_test_physics_body(
        HENKA_PHYSICS_BODY_DYNAMIC,
        henka_physics_collider_capsule(0.5f, 0.0f),
        (henka_vec3){0.0f, 0.25f, 0.0f});
    HENKA_TEST_ASSERT(henka_physics_body_create(world, &desc, &shape_body) == HENKA_SUCCESS);
    desc = henka_test_physics_body(
        HENKA_PHYSICS_BODY_STATIC,
...
static void henka_test_physics_scene_link_transaction(void)
{
    henka_scene* scene = NULL;
    henka_physics_world* world = NULL;
    henka_entity entities[2];
    henka_physics_body_id bodies[2];
    henka_physics_body_id rejected_body;
    henka_physics_body_desc desc;
    henka_physics_body_state before_states[2];
    henka_physics_body_state after_states[2];
    henka_transform before_scene_transforms[2];
    henka_transform after_scene_transforms[2];
    henka_transform attempted_transform;
    size_t body_count_before;
    size_t index;
    ...
    for (index = 0U; index < 2U; ++index)
    {
        entities[index] = henka_scene_create_entity_named(
            scene,
            index == 0U ? "Physics Transaction A" : "Physics Transaction B");
    ...
    desc = henka_test_physics_body(
        ...
        (henka_vec3){0.0f, 1.0f, 0.0f});
    desc.linked_scene = scene;
    desc.linked_entity = entities[0];
    HENKA_TEST_ASSERT(henka_physics_body_create(world, &desc, &bodies[0]) == HENKA_SUCCESS);
    desc.transform.position = (henka_vec3){2.0f, 1.0f, 0.0f};
    desc.linked_entity = entities[1];
    HENKA_TEST_ASSERT(henka_physics_body_create(world, &desc, &bodies[1]) == HENKA_SUCCESS);
    HENKA_TEST_ASSERT(henka_physics_body_set_linear_velocity(
        world,
        bodies[0],
        (henka_vec3){1.0f, 0.0f, 0.0f}) == HENKA_SUCCESS);
    HENKA_TEST_ASSERT(henka_physics_body_set_linear_velocity(
        world,
        bodies[1],
        (henka_vec3){-1.0f, 0.0f, 0.0f}) == HENKA_SUCCESS);
    ...
    for (index = 0U; index < 2U; ++index)
    {
        HENKA_TEST_ASSERT(henka_physics_body_get_state(
            world,
            bodies[index],
            &before_states[index]) == HENKA_SUCCESS);
        HENKA_TEST_ASSERT(henka_scene_get_entity_transform(
            scene,
            entities[index],
            &before_scene_transforms[index]) == HENKA_SUCCESS);
    }
    ...
    HENKA_TEST_ASSERT(henka_physics_world_step_fixed(world) == HENKA_ERROR_LIMIT);
    HENKA_TEST_ASSERT(henka_scene_get_render_revision(scene) == UINT64_MAX - UINT64_C(1));
    HENKA_TEST_ASSERT(scene->content_revision == UINT64_MAX - UINT64_C(1));
    ...
    for (index = 0U; index < 2U; ++index)
    {
        HENKA_TEST_ASSERT(henka_physics_body_get_state(
            world,
            bodies[index],
            &after_states[index]) == HENKA_SUCCESS);
        HENKA_TEST_ASSERT(henka_scene_get_entity_transform(
            scene,
            entities[index],
            &after_scene_transforms[index]) == HENKA_SUCCESS);
        HENKA_TEST_ASSERT_FLOAT_CLOSE(
            after_states[index].transform.position.x,
            before_states[index].transform.position.x,
            0.000001f);
        HENKA_TEST_ASSERT_FLOAT_CLOSE(
            after_scene_transforms[index].position.x,
            before_scene_transforms[index].position.x,
            0.000001f);
    }
    HENKA_TEST_ASSERT(henka_physics_world_get_body_count(world) == 2U);
    ...
    attempted_transform = before_states[0].transform;
    attempted_transform.position.x += 4.0f;
    HENKA_TEST_ASSERT(henka_physics_body_set_transform(
        world,
        bodies[0],
        attempted_transform,
        true) == HENKA_ERROR_LIMIT);
    HENKA_TEST_ASSERT(henka_physics_body_get_state(
        world,
        bodies[0],
        &after_states[0]) == HENKA_SUCCESS);
    HENKA_TEST_ASSERT_FLOAT_CLOSE(
        after_states[0].transform.position.x,
        before_states[0].transform.position.x,
        0.000001f);
    rejected_body = HENKA_INVALID_PHYSICS_BODY_ID;
    body_count_before = henka_physics_world_get_body_count(world);
    desc = henka_test_physics_body(
        HENKA_PHYSICS_BODY_DYNAMIC,
        henka_physics_collider_sphere(0.5f),
        (henka_vec3){50.0f, 1.0f, 0.0f});
    desc.linked_scene = scene;
    desc.linked_entity = entities[0];
    HENKA_TEST_ASSERT(henka_physics_body_create(
        world,
        &desc,
        &rejected_body) == HENKA_ERROR_LIMIT);
    HENKA_TEST_ASSERT(rejected_body == HENKA_INVALID_PHYSICS_BODY_ID);
    HENKA_TEST_ASSERT(henka_physics_world_get_body_count(world) == body_count_before);

    scene->render_revision = 100U;
    scene->content_revision = 100U;
    HENKA_TEST_ASSERT(henka_physics_world_step_fixed(world) == HENKA_SUCCESS);
    for (index = 0U; index < 2U; ++index)
    {
        HENKA_TEST_ASSERT(henka_physics_body_get_state(
        ...
        HENKA_TEST_ASSERT(henka_scene_get_entity_transform(
            ...
            entities[index],
            &after_scene_transforms[index]) == HENKA_SUCCESS);
        HENKA_TEST_ASSERT(after_states[index].transform.position.x !=
            before_states[index].transform.position.x);
        HENKA_TEST_ASSERT_FLOAT_CLOSE(
            after_scene_transforms[index].position.x,
            after_states[index].transform.position.x,
    ...
    scene->render_revision = UINT64_MAX;
    scene->content_revision = UINT64_MAX;
    HENKA_TEST_ASSERT(henka_physics_world_reset(world) == HENKA_ERROR_LIMIT);
    for (index = 0U; index < 2U; ++index)
    {
        henka_physics_body_state reset_state;
        henka_transform reset_scene_transform;

        HENKA_TEST_ASSERT(henka_physics_body_get_state(
            world,
            bodies[index],
            &reset_state) == HENKA_SUCCESS);
        HENKA_TEST_ASSERT(henka_scene_get_entity_transform(
            scene,
            entities[index],
            &reset_scene_transform) == HENKA_SUCCESS);
        HENKA_TEST_ASSERT_FLOAT_CLOSE(
            reset_state.transform.position.x,
            after_states[index].transform.position.x,
            0.000001f);
        HENKA_TEST_ASSERT_FLOAT_CLOSE(
            reset_scene_transform.position.x,
            after_scene_transforms[index].position.x,
            0.000001f);
    }
...
static void henka_test_physics_scene_link_survives_scene_destroy(void)
{
    henka_scene* scene = NULL;
    henka_entity entity;
    henka_physics_world* world = NULL;
    henka_physics_body_desc desc;
    henka_physics_body_id body = HENKA_INVALID_PHYSICS_BODY_ID;
    henka_physics_body_id candidate_body = HENKA_INVALID_PHYSICS_BODY_ID;
    henka_physics_body_state state;
    henka_transform transform;

    HENKA_TEST_ASSERT(henka_scene_create(&scene) == HENKA_SUCCESS);
    entity = henka_scene_create_entity_named(scene, "Destroyed Scene Link");
    HENKA_TEST_ASSERT(entity != HENKA_INVALID_ENTITY);
    HENKA_TEST_ASSERT(henka_physics_world_create(&world) == HENKA_SUCCESS);
    HENKA_TEST_ASSERT(henka_physics_world_set_gravity(
        world,
    ...
    HENKA_TEST_ASSERT(candidate_body == HENKA_INVALID_PHYSICS_BODY_ID);
    ...
    HENKA_TEST_ASSERT(henka_physics_body_destroy(world, body) == HENKA_SUCCESS);

    /* World destruction must release links for bodies not explicitly destroyed. */
    HENKA_TEST_ASSERT(henka_scene_create(&scene) == HENKA_SUCCESS);
    entity = henka_scene_create_entity_named(scene, "World Destroy Scene Link");
    HENKA_TEST_ASSERT(entity != HENKA_INVALID_ENTITY);
    desc.linked_scene = scene;
    desc.linked_entity = entity;
    HENKA_TEST_ASSERT(henka_physics_body_create(world, &desc, &body) == HENKA_SUCCESS);
...
static void henka_test_physics_transactional_allocation_failure(void)
{
    ...
    henka_memory_test_fail_after(8U);
...
static void henka_test_physics_sphere_overlap_query(void)
{
    ...
    desc = henka_test_physics_body(
        HENKA_PHYSICS_BODY_STATIC,
        henka_physics_collider_sphere(0.25f),
        (henka_vec3){0.0f, 0.0f, 0.6f});
    desc.collider.layer = 2U;
    desc.collider.is_trigger = true;
    HENKA_TEST_ASSERT(henka_physics_body_create(
        world, &desc, &trigger) == HENKA_SUCCESS);
    ...
    desc = henka_test_physics_body(
        ...
        henka_physics_collider_plane(
            (henka_vec3){0.0f, 1.0f, 0.0f},
            0.0f),
        (henka_vec3){0.0f, -0.5f, 0.0f});
    desc.collider.layer = 4U;
    HENKA_TEST_ASSERT(henka_physics_body_create(
        world, &desc, &plane) == HENKA_SUCCESS);

    HENKA_TEST_ASSERT(henka_physics_world_overlap_sphere(
        world,
        (henka_vec3){0.0f, 0.0f, 0.0f},
        0.6f,
        HENKA_PHYSICS_ALL_LAYERS,
    ...
    HENKA_TEST_ASSERT(henka_physics_world_overlap_sphere(
        ...
        limited,
        2U,
        &count) == HENKA_ERROR_LIMIT);
    HENKA_TEST_ASSERT(count == 3U);
    HENKA_TEST_ASSERT(limited[0] == 77U && limited[1] == 88U);
    ...
    for (index = 0U; index < count; ++index)
    {
        ...
        found_trigger = found_trigger || results[index] == trigger;
        found_plane = found_plane || results[index] == plane;
        HENKA_TEST_ASSERT(results[index] != box);
    }
    HENKA_TEST_ASSERT(found_sphere && found_trigger && found_plane);
    ...
    HENKA_TEST_ASSERT(henka_physics_world_overlap_sphere(
        ...
        4U,
        &count) == HENKA_SUCCESS);
    HENKA_TEST_ASSERT(count == 1U && results[0] == sphere);

    HENKA_TEST_ASSERT(henka_physics_body_destroy(
        world, trigger) == HENKA_SUCCESS);
    count = 0U;
    HENKA_TEST_ASSERT(henka_physics_world_overlap_sphere(
        world,
        (henka_vec3){0.0f, 0.0f, 0.0f},
...
void henka_test_physics(void)
{
    ...
    HENKA_TEST_ASSERT(strcmp(henka_physics_body_type_get_label(HENKA_PHYSICS_BODY_STATIC), "Static") == 0);
    HENKA_TEST_ASSERT(strcmp(henka_physics_body_type_get_label(HENKA_PHYSICS_BODY_KINEMATIC), "Kinematic") == 0);
    HENKA_TEST_ASSERT(strcmp(henka_physics_shape_type_get_label(HENKA_PHYSICS_SHAPE_BOX), "AABB") == 0);
    HENKA_TEST_ASSERT(strcmp(henka_physics_shape_type_get_label(HENKA_PHYSICS_SHAPE_SPHERE), "Sphere") == 0);
    HENKA_TEST_ASSERT(strcmp(henka_physics_shape_type_get_label(HENKA_PHYSICS_SHAPE_PLANE), "Plane") == 0);
    HENKA_TEST_ASSERT(strcmp(henka_physics_shape_type_get_label(HENKA_PHYSICS_SHAPE_HEIGHTFIELD), "Heightfield") == 0);
    HENKA_TEST_ASSERT(strcmp(henka_physics_shape_type_get_label(HENKA_PHYSICS_SHAPE_CAPSULE), "Capsule") == 0);
    HENKA_TEST_ASSERT(strcmp(henka_physics_event_type_get_label(HENKA_PHYSICS_EVENT_COLLISION_ENTER), "Collision Enter") == 0);
    HENKA_TEST_ASSERT(strcmp(henka_physics_event_type_get_label(HENKA_PHYSICS_EVENT_COLLISION_STAY), "Collision Stay") == 0);
    HENKA_TEST_ASSERT(strcmp(henka_physics_event_type_get_label(HENKA_PHYSICS_EVENT_COLLISION_EXIT), "Collision Exit") == 0);
    HENKA_TEST_ASSERT(strcmp(henka_physics_event_type_get_label(HENKA_PHYSICS_EVENT_TRIGGER_ENTER), "Trigger Enter") == 0);
    HENKA_TEST_ASSERT(strcmp(henka_physics_event_type_get_label(HENKA_PHYSICS_EVENT_TRIGGER_STAY), "Trigger Stay") == 0);
    HENKA_TEST_ASSERT(strcmp(henka_physics_event_type_get_label(HENKA_PHYSICS_EVENT_TRIGGER_EXIT), "Trigger Exit") == 0);
    HENKA_TEST_ASSERT(strcmp(henka_physics_event_type_get_label((henka_physics_event_type)-1), "Unknown") == 0);
    henka_test_physics_motion_and_materials();
    henka_test_physics_direct_state_setters();
    henka_test_physics_world_configuration();
