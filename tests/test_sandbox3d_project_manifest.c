...
static int test_project_reopen_materializes_authoring_mesh(void)
{
    ...
    henka_authoring_mesh* replacement_mesh = NULL;
    henka_scene* source_scene = NULL;
    henka_scene* reopened_scene = NULL;
    ...
    henka_mesh* reopened_mesh = NULL;
    henka_mesh* reloaded_mesh = NULL;
    henka_mesh* continued_mesh = NULL;
    ...
    if (henka_authoring_mesh_create_box(
            ...
            7.0f,
            8.0f,
            &replacement_mesh) != HENKA_SUCCESS ||
        replacement_mesh == NULL ||
        henka_authoring_mesh_save_file(replacement_mesh, source_path) !=
            HENKA_SUCCESS)
    {
        goto cleanup;
    }
    henka_authoring_mesh_destroy(replacement_mesh);
    replacement_mesh = NULL;
    if (sandbox3d_game_authoring_load(reopened_authoring, project_root) !=
            HENKA_SUCCESS ||
        henka_scene_get_entity_mesh(
            reopened_scene, reopened_entity, &reloaded_mesh) != HENKA_SUCCESS ||
        reloaded_mesh == NULL ||
        reloaded_mesh == reopened_mesh)
    {
        goto cleanup;
    }
    {
        henka_mesh* stable_mesh = reloaded_mesh;
        const uint64_t stable_render_revision =
            henka_scene_get_render_revision(reopened_scene);
        const bool wrote_invalid_source =
            test_write_file(source_path, "not a valid HAMS file\n");
        const henka_result failed_reload_result = wrote_invalid_source
            ? sandbox3d_game_authoring_load(reopened_authoring, project_root)
            : HENKA_ERROR_UNKNOWN;
        const henka_result failed_mesh_result = henka_scene_get_entity_mesh(
            reopened_scene, reopened_entity, &reloaded_mesh);
        const uint64_t failed_render_revision =
            henka_scene_get_render_revision(reopened_scene);
        if (!wrote_invalid_source || failed_reload_result == HENKA_SUCCESS ||
            failed_mesh_result != HENKA_SUCCESS || reloaded_mesh == NULL ||
            reloaded_mesh != stable_mesh ||
            failed_render_revision != stable_render_revision)
        {
            goto cleanup;
        }
    }
    ...
    object.transform.position.x = 4.5f;
    (void)snprintf(
        object.name,
        sizeof(object.name),
        "%s",
        "Continued Persisted Authoring Mesh");
    if (sandbox3d_game_authoring_update_object_for_entity(
            reopened_authoring,
            reopened_entity,
            &object) != HENKA_SUCCESS ||
        sandbox3d_game_authoring_save(reopened_authoring, project_root) !=
            HENKA_SUCCESS)
    {
        goto cleanup;
    }
    ...
    if (henka_engine_create(&config, &engine) != HENKA_SUCCESS ||
        sandbox3d_game_authoring_open_project_with_engine(
            ...
            engine,
            &continued_scene,
            &continued_authoring) != HENKA_SUCCESS ||
        continued_scene == NULL || continued_authoring == NULL ||
        sandbox3d_game_authoring_get_entity_for_document_id(
            continued_authoring,
            document_id,
            &continued_entity) != HENKA_SUCCESS ||
        sandbox3d_game_authoring_get_object_for_entity(
            continued_authoring,
            continued_entity,
            &continued_id,
            &continued_object) != HENKA_SUCCESS ||
        continued_id != document_id ||
        strcmp(continued_object.name, "Continued Persisted Authoring Mesh") != 0 ||
        !test_float_close(continued_object.transform.position.x, 4.5f) ||
        continued_object.source.kind !=
            HENKA_SCENE_DOCUMENT_SOURCE_AUTHORING_MESH ||
        strcmp(continued_object.source.path, "authored_source.hams") != 0 ||
        henka_scene_get_entity_mesh(
            continued_scene,
            continued_entity,
            &continued_mesh) != HENKA_SUCCESS ||
        continued_mesh == NULL)
    {
        goto cleanup;
    }
...
static int test_project_load_materializes_authoring_mesh_with_engine_context(void)
{
    const char* project_root =
        "build/test_tmp/project_authoring_mesh_load_context_root";
    const char* scene_path = "authoring_mesh_scene.hscene";
    const char* source_path =
        "build/test_tmp/project_authoring_mesh_load_context_root/authored_source.hams";
    const char* manifest_path =
        "build/test_tmp/project_authoring_mesh_load_context_root/henka.project";
    const char* scene_file_path =
        "build/test_tmp/project_authoring_mesh_load_context_root/authoring_mesh_scene.hscene";
    henka_authoring_mesh* source_mesh = NULL;
    henka_scene* source_scene = NULL;
    henka_scene* runtime_scene = NULL;
    sandbox3d_game_authoring* source_authoring = NULL;
    sandbox3d_game_authoring* runtime_authoring = NULL;
    henka_engine* engine = NULL;
    henka_engine_config config = {0};
    henka_authoring_mesh_desc description = henka_authoring_mesh_desc_default();
    henka_entity source_entity = HENKA_INVALID_ENTITY;
    henka_entity runtime_entity = HENKA_INVALID_ENTITY;
    henka_scene_document_id source_id = HENKA_INVALID_SCENE_DOCUMENT_ID;
    henka_scene_document_id runtime_id = HENKA_INVALID_SCENE_DOCUMENT_ID;
    henka_scene_document_object object;
    henka_mesh* runtime_mesh = NULL;
    henka_camera camera;
    int success = 0;
    ...
    if (!test_ensure_directory("build/test_tmp") ||
        ...
        henka_authoring_mesh_create_box(
            &description,
            2.0f,
            3.0f,
            4.0f,
    ...
    if (henka_scene_create(&source_scene) != HENKA_SUCCESS ||
        ...
        (source_entity = henka_scene_create_entity_named(
            source_scene, "Persisted Startup Mesh")) == HENKA_INVALID_ENTITY ||
        sandbox3d_game_authoring_create(
            source_scene, scene_path, &source_authoring) != HENKA_SUCCESS ||
        sandbox3d_game_authoring_register_entity(
            source_authoring, source_entity, &source_id) != HENKA_SUCCESS ||
        sandbox3d_game_authoring_get_object_for_entity(
            source_authoring, source_entity, &source_id, &object) != HENKA_SUCCESS)
    {
        goto cleanup;
    }
    object.source.kind = HENKA_SCENE_DOCUMENT_SOURCE_AUTHORING_MESH;
    ...
    config.application_name = "Henka Project Load Context Test";
    config.window_width = 320;
    config.window_height = 240;
    config.enable_vsync = false;
    config.asset_base_path = ".";
    if (henka_engine_create(&config, &engine) != HENKA_SUCCESS ||
        henka_scene_create(&runtime_scene) != HENKA_SUCCESS ||
        henka_scene_set_camera(runtime_scene, &camera) != HENKA_SUCCESS ||
        (runtime_entity = henka_scene_create_entity_named(
            runtime_scene, "Runtime Startup Entity")) == HENKA_INVALID_ENTITY ||
        sandbox3d_game_authoring_create_with_engine(
            runtime_scene,
            scene_path,
            engine,
            &runtime_authoring) != HENKA_SUCCESS ||
        sandbox3d_game_authoring_register_entity(
            runtime_authoring, runtime_entity, &runtime_id) != HENKA_SUCCESS ||
        runtime_id != source_id ||
        sandbox3d_game_authoring_load(runtime_authoring, project_root) !=
            HENKA_SUCCESS ||
        henka_scene_get_entity_mesh(runtime_scene, runtime_entity, &runtime_mesh) !=
            HENKA_SUCCESS ||
        runtime_mesh == NULL ||
        sandbox3d_game_authoring_get_object_for_entity(
            runtime_authoring, runtime_entity, &runtime_id, &object) != HENKA_SUCCESS ||
        strcmp(object.name, "Persisted Startup Mesh") != 0)
    {
        goto cleanup;
    }
    success = 1;
...
cleanup:
    sandbox3d_game_authoring_destroy(runtime_authoring);
    sandbox3d_game_authoring_destroy(source_authoring);
    henka_scene_destroy(runtime_scene);
    henka_scene_destroy(source_scene);
    henka_authoring_mesh_destroy(source_mesh);
    henka_engine_destroy(engine);
    (void)remove(source_path);
...
{
    const char* project_root =
        "build/test_tmp/project_open_continue_root";
    const char* scene_path = "authored_project.hscene";
    const char* source_path =
        "build/test_tmp/project_open_continue_root/authored_source.hams";
    const char* manifest_path =
        "build/test_tmp/project_open_continue_root/henka.project";
    const char* scene_file_path =
        "build/test_tmp/project_open_continue_root/authored_project.hscene";
    henka_authoring_mesh* source_mesh = NULL;
    henka_scene* source_scene = NULL;
    ...
    henka_authoring_mesh_desc description = henka_authoring_mesh_desc_default();
    henka_entity source_entity = HENKA_INVALID_ENTITY;
    henka_entity first_entity = HENKA_INVALID_ENTITY;
    henka_entity second_entity = HENKA_INVALID_ENTITY;
    henka_scene_document_id source_id = HENKA_INVALID_SCENE_DOCUMENT_ID;
    henka_scene_document_id first_id = HENKA_INVALID_SCENE_DOCUMENT_ID;
    henka_scene_document_id second_id = HENKA_INVALID_SCENE_DOCUMENT_ID;
    henka_scene_document_object object;
    henka_mesh* first_mesh = NULL;
    henka_mesh* second_mesh = NULL;
    henka_camera camera;
    int success = 0;
    ...
    if (henka_scene_create(&source_scene) != HENKA_SUCCESS ||
        henka_scene_set_camera(source_scene, &camera) != HENKA_SUCCESS ||
        (source_entity = henka_scene_create_entity_named(
            source_scene, "Open Project Mesh")) == HENKA_INVALID_ENTITY ||
        sandbox3d_game_authoring_create(
            source_scene, scene_path, &source_authoring) != HENKA_SUCCESS ||
        sandbox3d_game_authoring_register_entity(
            source_authoring, source_entity, &source_id) != HENKA_SUCCESS ||
    ...
    config.application_name = "Henka Project Open Continue Test";
    config.window_width = 320;
    config.window_height = 240;
    config.enable_vsync = false;
    config.asset_base_path = ".";
    if (henka_engine_create(&config, &engine) != HENKA_SUCCESS ||
        ...
        sandbox3d_game_authoring_get_entity_for_document_id(
            first_authoring, source_id, &first_entity) != HENKA_SUCCESS ||
        !henka_scene_is_entity_valid(first_scene, first_entity) ||
        henka_scene_get_entity_mesh(first_scene, first_entity, &first_mesh) !=
            HENKA_SUCCESS ||
        first_mesh == NULL ||
        sandbox3d_game_authoring_get_object_for_entity(
            first_authoring, first_entity, &first_id, &object) != HENKA_SUCCESS ||
        first_id != source_id ||
        strcmp(object.name, "Open Project Mesh") != 0)
    {
        goto cleanup;
    }

    object.transform.position.x = 7.25f;
    (void)snprintf(
        object.name,
        sizeof(object.name),
        "%s",
        "Continued Open Project Mesh");
    if (sandbox3d_game_authoring_update_object_for_entity(
            first_authoring, first_entity, &object) != HENKA_SUCCESS ||
        sandbox3d_game_authoring_save(first_authoring, project_root) != HENKA_SUCCESS)
    {
        goto cleanup;
    }
    ...
    if (sandbox3d_game_authoring_open_project_with_engine(
        ...
        sandbox3d_game_authoring_get_entity_for_document_id(
            second_authoring, source_id, &second_entity) != HENKA_SUCCESS ||
        !henka_scene_is_entity_valid(second_scene, second_entity) ||
        henka_scene_get_entity_mesh(second_scene, second_entity, &second_mesh) !=
            HENKA_SUCCESS ||
        second_mesh == NULL ||
        sandbox3d_game_authoring_get_object_for_entity(
            second_authoring, second_entity, &second_id, &object) != HENKA_SUCCESS ||
        second_id != source_id ||
        strcmp(object.name, "Continued Open Project Mesh") != 0 ||
        !test_float_close(object.transform.position.x, 7.25f))
    {
        goto cleanup;
    }
    success = 1;
...
int main(void)
{
    ...
    {
        fprintf(stderr, "project open/reopen authoring mesh edit cycle failed\n");
        goto cleanup;
    }
