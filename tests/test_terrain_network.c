...
static int test_request_and_response_codecs(void)
{
    ...
    henka_terrain_snapshot_failure snapshot_failure = {0};
    henka_terrain_snapshot_failure decoded_snapshot_failure;
    ...
    snapshot_failure.world_identity = 11U;
    snapshot_failure.base_asset_identity = 22U;
    snapshot_failure.region_id = snapshot_request.region_id;
    snapshot_failure.expected_revision = 8U;
    snapshot_failure.reason = HENKA_TERRAIN_SNAPSHOT_FAILURE_OUT_OF_MEMORY;
    if (henka_terrain_snapshot_failure_encode(
            &snapshot_failure, buffer, sizeof(buffer), &size) != HENKA_SUCCESS ||
        henka_terrain_snapshot_failure_decode(
            buffer, size, &decoded_snapshot_failure) != HENKA_SUCCESS ||
        decoded_snapshot_failure.region_id.z != 3 ||
        decoded_snapshot_failure.reason != HENKA_TERRAIN_SNAPSHOT_FAILURE_OUT_OF_MEMORY)
    {
        return 0;
    }
    {
    ...
    session_info.regions[0] = (henka_terrain_session_region){{2, 3}, 8U, 9U};
    if (henka_terrain_session_info_encode(
        ...
        decoded_session_info.region_count != 1U ||
    ...
    buffer[16U] = (uint8_t)(HENKA_TERRAIN_NETWORK_MAX_SESSION_REGIONS + 1U);
    if (henka_terrain_session_info_decode(
            buffer, size, &decoded_session_info) != HENKA_ERROR_INVALID_ARGUMENT)
    {
        return 0;
    }
    session_info.region_count = HENKA_TERRAIN_NETWORK_MAX_SESSION_REGIONS + 1U;
    if (henka_terrain_session_info_encode(
            &session_info, buffer, sizeof(buffer), &size) != HENKA_ERROR_INVALID_ARGUMENT)
    {
        return 0;
    }
    recovery_request.world_identity = 11U;
    recovery_request.base_asset_identity = 22U;
    recovery_request.region_id = (henka_terrain_region_id){2, 3};
    recovery_request.from_revision = 4U;
    recovery_request.target_revision = 8U;
    if (henka_terrain_delta_recovery_request_encode(
            &recovery_request, buffer, sizeof(buffer), &size) != HENKA_SUCCESS ||
        henka_terrain_delta_recovery_request_decode(
            buffer, size, &decoded_recovery_request) != HENKA_SUCCESS ||
        decoded_recovery_request.region_id.z != 3 ||
        decoded_recovery_request.from_revision != 4U ||
