...
void henka_test_memory(void)
{
    ...
    before_count = henka_memory_get_allocation_count();
    memset(&before_diagnostics, 0, sizeof(before_diagnostics));
    henka_memory_get_diagnostics(&before_diagnostics);
    HENKA_TEST_ASSERT(before_diagnostics.active_allocation_count == before_count);
    HENKA_TEST_ASSERT(
        before_diagnostics.peak_active_allocation_count >= before_count);
    ...
    block = henka_malloc(8U);
    second_block = henka_calloc(2U, 8U);
    HENKA_TEST_ASSERT(block != NULL && second_block != NULL);
    henka_memory_get_diagnostics(&during_diagnostics);
    HENKA_TEST_ASSERT(
        during_diagnostics.active_allocation_count == before_count + 2U);
    HENKA_TEST_ASSERT(
        during_diagnostics.peak_active_allocation_count >= before_count + 2U);
    HENKA_TEST_ASSERT(
        during_diagnostics.successful_allocation_count >=
        before_diagnostics.successful_allocation_count + 2U);
    henka_free(second_block);
    henka_free(block);
    henka_memory_get_diagnostics(&after_diagnostics);
    HENKA_TEST_ASSERT(after_diagnostics.active_allocation_count == before_count);
    HENKA_TEST_ASSERT(
        after_diagnostics.peak_active_allocation_count >=
        during_diagnostics.peak_active_allocation_count);
    HENKA_TEST_ASSERT(
