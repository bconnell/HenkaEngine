...
- Sandbox smoke additionally sends one shared integer raise command through the Terrain world, automatically refreshes the stale render mesh revision, and replaces the bounded collision patch transactionally; the Terrain utility can save the resident region through the shared journal and compact committed history, while networked edit authority remains a separate integration
