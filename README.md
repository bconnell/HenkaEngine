...
- The opt-in Windows sandbox command `--residency-stress` exercises the public asset path with 65 path-distinct manager-owned textures, shared-cache identity, configured-budget rejection, active-frame pinning, full-queue saturation and recovery, failed request processing, cancellation, and shutdown cleanup. It is a lifetime and queue-pressure scenario using packaged PNG fixtures; KTX2 promotion/demotion, compressed-format pressure, visibility threshold transitions, and background I/O remain separate unfinished validation tracks
