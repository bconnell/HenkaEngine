...
- Renderer-independent Terrain server session routing decodes bounded edit requests from the public ENet server, invokes authority validation, sends encoded acceptance/rejection responses, echoes control pings, disconnects malformed edit peers, and exposes dispatch diagnostics; client replication, reconnect recovery, and prediction remain unfinished
- Authoritative Terrain edit deltas use explicit bounded world/base identity, client nonce, server command ID, deterministic command fields, and resulting region revisions; accepted deltas are broadcast reliably to connected peers, while client application, snapshots, reconnect recovery, and prediction remain unfinished
- Dedicated-server Terrain snapshot serving validates bounded requests, reads a checksummed region record, and sends transfer-identified fragments below the 32 KiB snapshot ceiling; client fragment assembly, reconnect/late-join state recovery, and prediction remain unfinished
