...
- Complete cross-backend KTX2/Basis production coverage and GPU-native stress validation, background texture streaming/eviction, production instancing, SSR, motion-vector TAA, and full refraction/volume/glass rendering remain unfinished; the current glTF transmission factor uses a bounded environment response. Point-light shadow cubemaps remain unfinished. Current paths expose truthful fallbacks or bounded foundations
