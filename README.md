...
- The opt-in Windows sandbox command `--temporal-stress` drives the public scene path through a viewport resize and restore, camera translation, projection change, entity disocclusion and reappearance, and recovery frames; it reports the existing resolve/fallback/invalidation diagnostics but does not claim production TAA visual closure
...
- The public `--residency-stress` scenario also generates a native BC1 KTX2 mip chain. When the active device exposes BC1, it proves compressed residency trim and promotion; otherwise the native-compressed source fails closed and the scenario reports that capability result. This is compressed-format pressure coverage, not asynchronous streaming or a complete budget-convergence policy
