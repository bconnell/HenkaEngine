...
- Version-1 bounded Henka network packet codec and localhost ENet transport with explicit little-endian headers, three logical channels, reliable ordered delivery, a 64 KiB packet ceiling, 32 KiB snapshot fragments, authoritative Terrain deltas/snapshots, and a renderer-free client session adapter; reconnect, late-join orchestration, prediction, and multiplayer soak validation remain unfinished
...
- Bounded Terrain authority validation checks peer rate limits, permissions, world/base identity, exact affected-region sets, and expected revisions, applies deterministic edits, commits all affected region snapshots before returning acceptance, and restores live samples/revisions on persistence failure; replication recovery and client prediction remain unfinished
- Renderer-independent Terrain server session routing decodes bounded edit requests from the public ENet server, invokes authority validation, sends encoded acceptance/rejection responses, echoes control pings, disconnects malformed edit peers, and exposes dispatch diagnostics; reconnect recovery, prediction, and editor controls remain unfinished
- Authoritative Terrain edit deltas use explicit bounded world/base identity, client nonce, server command ID, deterministic command fields, and resulting region revisions; accepted deltas are broadcast reliably to connected peers and the client adapter applies exact-revision deltas, while reconnect recovery and prediction remain unfinished
- Dedicated-server Terrain snapshot serving validates bounded requests, reads a checksummed region record, and sends transfer-identified fragments below the 32 KiB snapshot ceiling; the client adapter assembles and atomically applies those fragments, while reconnect/late-join orchestration and prediction remain unfinished
- Renderer-independent Terrain client session ownership routes public network events through the bounded replica, sends edit/snapshot requests, records acceptance/rejection diagnostics, and requests region snapshots when a delta gap or nonresident region requires recovery; prediction, reconnect policy, and editor controls remain unfinished
