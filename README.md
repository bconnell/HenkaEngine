...
- Bounded Windows Terrain streaming foundation with a renderer-free worker, coalesced region requests, fixed request/completion queues, observer records, cancellation, candidate validation, main-thread region swaps, observer-driven CPU/physics/render residency flags, deterministic clean-region eviction, optional unload hysteresis, current and high-water queue/observer diagnostics, and fail-closed saturation counters; the Sandbox now seeds a persistent 2x2 procedural fixture in a four-region CPU budget, feeds the active camera into that observer, and the opt-in `--terrain-stream-stress` path proves a rendered `(0,0) -> (1,0) -> (0,0)` transition with no failed requests, while broad-world regeneration and background physics/render regeneration remain unfinished
