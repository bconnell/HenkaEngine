...
- Version-1 bounded Henka network packet codec and localhost ENet transport with explicit little-endian headers, three logical channels, reliable ordered delivery, a 64 KiB packet ceiling, 32 KiB snapshot fragments, authoritative Terrain deltas/snapshots, and a renderer-free client session adapter; resident-region snapshot requests now begin on reconnect, while handshake identity exchange, broader late-join orchestration, and multiplayer soak validation remain unfinished
...
- Renderer-independent Terrain server session routing decodes bounded edit requests from the public ENet server, invokes authority validation, sends encoded acceptance/rejection responses, echoes control pings, disconnects malformed edit peers, and exposes dispatch diagnostics; reconnect recovery and editor controls remain unfinished
- Authoritative Terrain edit deltas use explicit bounded world/base identity, client nonce, server command ID, deterministic command fields, and resulting region revisions; accepted deltas are broadcast reliably to connected peers, the client adapter applies exact-revision deltas, and bounded pending edits replay over authoritative state, while reconnect recovery remains unfinished
- Dedicated-server Terrain snapshot serving validates bounded requests, reads the live authoritative region (materializing a persisted region when needed), and sends transfer-identified fragments below the 32 KiB snapshot ceiling; the client adapter assembles and atomically applies those fragments, while broader late-join orchestration remains unfinished
- Renderer-independent Terrain client session ownership routes public network events through the bounded replica, sends edit/snapshot requests, records acceptance/rejection diagnostics, replays a bounded predicted world over authoritative state, requests region snapshots when a delta gap or nonresident region requires recovery, and requests resident-region snapshots on reconnect; handshake identity exchange and editor controls remain unfinished
