...
- Detached panel placement and resized dock widths are session-only; `Reset Layout` is the recovery path. The workspace now opens the required section context menu and executes bounded close, merge/tab-group, equalize, maximize/restore, and native-detach actions. Its horizontal/vertical window entries record explicit requests while dynamic section creation is completed; the existing sandbox renderer still projects its compatible side-dock view and does not yet expose the full topology editor workflow.
