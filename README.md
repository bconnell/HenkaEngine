...
- Detached panel placement and resized dock widths are session-only; `Reset Layout` is the recovery path. The bounded topology graph, split ratios, tab membership, closed-section mask, and maximize state now use a versioned validated settings snapshot; malformed or incompatible snapshots keep defaults. The workspace header chrome opens the required section context menu without stealing tool-content right-clicks, and executes bounded close, merge/tab-group, equalize, maximize/restore, native-detach, and last-closed restoration actions. Its horizontal/vertical window entries record explicit requests while dynamic section creation is completed; named layouts and the full topology editor workflow remain open.
