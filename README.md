...
- Sandbox smoke additionally sends one shared integer raise command through the Terrain world, automatically refreshes the stale render mesh revision, replaces the bounded collision patch transactionally, and forces a failed candidate mesh replacement to prove the previous mesh/revision stays live before recovery; the Terrain utility can save and transactionally reload the resident region through the shared journal, compact committed history, and preserve the prior presentation on reload failure, while networked edit authority remains a separate integration
