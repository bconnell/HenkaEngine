...
- Renderer-independent Terrain collision patch extraction copies a physics-resident chunk into a bounded 65 x 65 signed-millimeter height patch with revision and generation identity; a bounded transactional Terrain physics patch owner retains replacements and answers deterministic height/normal queries, the rigid-body API owns copied static heightfield colliders with sphere/box contacts and bounded raycasts, and a renderer-free coalescing rebuild queue synchronizes missing/stale physics-resident chunks and replaces patches transactionally; accepted edits now derive one-chunk neighbor coverage through the server session, while coverage beyond physics capacity and runtime collision stress validation remain unfinished
