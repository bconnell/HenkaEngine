...
- Version-1 bounded Henka network packet codec and localhost ENet transport with explicit little-endian headers, three logical channels, reliable ordered delivery, a 64 KiB packet ceiling, 32 KiB snapshot fragments, authoritative Terrain deltas/snapshots, and a renderer-free client session adapter; a bounded control session-info message validates world/base identity and advertises current resident regions so empty clients can bootstrap late-join snapshots, while application handshake/authentication, relevance-based region selection, reconnect policy beyond the advertised bound, and multiplayer soak validation remain unfinished
- Terrain v1 core contract with an 8192 m x 8192 m default descriptor, 512 m regions, 64 m chunks, 65 x 65 full-resolution chunk samples, signed millimeter heights, deterministic four-layer weight normalization, integer region/chunk identities, and separately bounded CPU/physics/render residency state; full residency-wide collision scheduling and automatic render-residency scheduling remain unfinished
...
- Bounded Terrain authority validation checks peer rate limits, permissions, world/base identity, exact affected-region sets, and expected revisions, applies deterministic edits, commits all affected region snapshots before returning acceptance, and restores live samples/revisions on persistence failure; reconnect and historical delta recovery remain unfinished
- Renderer-independent Terrain server session routing decodes bounded edit requests from the public ENet server, invokes authority validation, sends encoded acceptance/rejection responses, echoes control pings, sends bounded world/base session info with current resident-region revisions on connect, disconnects malformed edit peers, and exposes dispatch diagnostics; application handshake/authentication, relevance-based late-join selection, reconnect policy beyond the advertised bound, and editor controls remain unfinished
- Authoritative Terrain edit deltas use explicit bounded world/base identity, client nonce, server command ID, deterministic command fields, and resulting region revisions; accepted deltas are broadcast reliably to connected peers, the client adapter applies exact-revision deltas, and bounded pending edits replay over authoritative state, while broader relevance-driven reconnect recovery remains unfinished
...
- Renderer-independent Terrain client session ownership routes public network events through the bounded replica, validates connect-time session-info world/base identity, requests snapshots only for advertised regions whose local revision/generation is missing or stale, sends edit/snapshot requests, records acceptance/rejection diagnostics, replays a bounded predicted world over authoritative state, and requests region snapshots when a delta gap or nonresident region requires recovery; application handshake/authentication, broader relevance selection, reconnect policy beyond the advertised set, and editor controls remain unfinished
- Dedicated-server edit handling lazily materializes requested persisted regions within the configured residency bound before authority validation, avoiding a world-sized preload; streaming now has observer-driven bounded CPU-region eviction with optional unload hysteresis, while asynchronous physics/render regeneration remains unfinished
...
- Deterministic allocation-free Terrain LOD selection scans render-resident chunks in stable region/chunk order, applies bounded distance bands through LOD 3, reports considered/culled/selected counts, and fails closed on output limits; the graphical terrain owner adds fixed-capacity scene entities, renderer bounds culling, hysteretic four-band selection, deterministic adjacent-chunk LOD clamping, bounded GPU edge skirts, and transactional mesh replacement, while full neighbor-aware stitching and visual validation remain unfinished
- Renderer-independent Terrain chunk mesh generation converts a render-resident chunk into caller-owned LOD 0-3 vertices and indices with finite height-derived normals, world UVs, material weights, and source revision/generation identity; the graphical mesh API uploads that bounded data through the existing renderer and `<henka/terrain_render.h>` owns borrowed-world scene integration, bounded render slots, and deterministic mesh destruction
- The Sandbox reference scene exercises a deterministic four-chunk render-resident Terrain region through the graphical owner and camera-driven LOD update; the Terrain utility exposes bounded sculpt/paint controls through the shared command API, while viewport ray-pick, full neighbor-aware stitching, automatic residency scheduling, and human visual approval remain unfinished
- Sandbox smoke additionally sends one shared integer raise command through the Terrain world, refreshes the render mesh revision, and replaces the bounded collision patch transactionally; viewport editor interaction, persistence controls, and networked edit authority remain separate unfinished integrations
- Renderer-independent Terrain collision patch extraction copies a physics-resident chunk into a bounded 65 x 65 signed-millimeter height patch with revision and generation identity; a bounded transactional Terrain physics patch owner retains replacements and answers deterministic height/normal queries, the rigid-body API owns copied static heightfield colliders with sphere/box contacts and bounded raycasts, and a renderer-free coalescing rebuild queue replaces resident chunk patches transactionally; accepted edits now derive one-chunk neighbor coverage through the server session, while full residency-wide scheduling and runtime collision stress validation remain unfinished
