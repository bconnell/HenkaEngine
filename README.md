...
- Detached panel placement and resized dock widths are session-only; `Reset Layout` is the recovery path. The bounded topology graph, split ratios, tab membership, closed-section mask, maximize state, and active tab now use a versioned validated settings snapshot; malformed or incompatible snapshots keep defaults. The workspace header chrome opens the required section context menu without stealing tool-content right-clicks, and executes bounded close, merge/tab-group, equalize, maximize/restore, native-detach, last-closed restoration, and available-singleton split actions. Dock stack section order and membership now project from the validated topology, including suppression of closed and merged-away sections. Merged sections expose bounded header tab controls and project the selected tab’s content into the section; drag-to-tab, named layouts, and saved detached placement remain open.
