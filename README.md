...
- Version-1 bounded Henka network packet codec and localhost ENet transport with explicit little-endian headers, three logical channels, reliable ordered delivery, a 64 KiB packet ceiling, 32 KiB snapshot fragments, authoritative Terrain deltas/snapshots, renderer-free client session ownership, and explicit client reconnect; a bounded control session-info message validates world/base identity and advertises current resident regions so empty clients can bootstrap late-join snapshots, while application handshake/authentication, relevance-based region selection, reconnect policy beyond the advertised bound, and multiplayer soak validation remain unfinished
