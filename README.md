...
- Detached panel placement and resized dock widths are session-only; `Reset Layout` is the recovery path. The new split-topology model, divider transactions, stable section identity, and exact section context-command labels are bounded workspace foundations; the existing sandbox renderer still projects its compatible side-dock view and does not yet expose the full topology editor workflow.
