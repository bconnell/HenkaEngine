...
- Shared deterministic Terrain edit commands for raise/lower, flatten, smooth, and paint use algorithm version 1, integer sample centers/radii, fixed-point falloffs, candidate-region preflight, atomic multi-region swaps, revisions, and dirty state; editor tool integration remains unfinished
...
- The Sandbox reference scene exercises a deterministic four-chunk render-resident Terrain region through the graphical owner and camera-driven LOD update; editor sculpt/paint controls, full neighbor-aware stitching, automatic residency scheduling, and human visual approval remain unfinished
- Sandbox smoke additionally sends one shared integer raise command through the Terrain world, refreshes the render mesh revision, and replaces the bounded collision patch transactionally; editor interaction, persistence, and networked edit authority remain separate unfinished integrations
