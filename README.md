...
- The optional terrain visual evidence capture uses one deterministic reference camera aimed across the seeded rolling/valley/cliff area across Solid, Material Preview, and Rendered without changing scene materials or scene lights; capture mode refreshes the four bounded fixture regions from their seeded samples so persisted editor edits cannot change the comparison; it is application-only evidence for the rolling/valley/cliff/four-layer fixture, while human visual approval remains unfinished
