...
- explicit Entity, Transform, Input, Physics, Interaction, and Events domains;
...
Both operations resolve through the shared typed host API. Missing required providers produce an execution error in the HenkaScript path.
...
- checked host access through Entity, Transform, Input, Physics, Interaction, and Events tables;
...
When the Sandbox Play coordinator supplies the live input engine and camera observer snapshot, all six current domains are operational:
...
- Lua `Input.IsActionDown` returns `false`;
- Lua `Interaction.Try` returns `UNAVAILABLE`;
