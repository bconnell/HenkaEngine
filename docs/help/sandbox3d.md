...
- Save Project writes the bounded native source and optional owned-material sidecar into the confined sandbox user slot; a later normal sandbox launch restores valid saved showcase authoring state, while malformed or missing state keeps the imported render.
