...
- For an imported Showcase Giraffe or Showcase Rocket, use Object Details > Authoring > Make Editable, then Refine Profile to apply a bounded native region transform to the giraffe neck/head or rocket upper stage. The operation preserves topology and material regions while updating evaluated bounds and the native undo/redo history. Own Material then promotes a manager-owned definition: its compact controls edit base color, metallic, roughness, emissive strength, IOR, transmission, subsurface amount, subsurface thickness, and subsurface tint, and create bounded procedural detail-normal plus metallic-roughness textures; the nearby Move X+/Move Y+/Move Z+ controls operate on the selected native components. Undo Mat and Redo Mat restore those material-instance states transactionally before Save Project and Reload Project.
- In Scene Objects, use Create Native Rocket to create a 201-vertex/121-face multi-part rocket directly through the Henka authoring mesh path. Its native source includes a continuous body/nose, three structural collars, a five-bell engine cluster, and four fins; the action also seeds manager-owned normal and metallic/roughness detail textures when GPU texture creation succeeds. It becomes the selected native source with a manager-owned material instance and supports the same bounded component, topology, material, and project save/reload workflow; the packaged relaunch path restores the valid saved source and material sidecar transactionally.
