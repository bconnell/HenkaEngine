...
The Utility Diagnostics panel includes the active shading/exposure row plus current texture-residency bytes, configured budget, and queued request count from the engine diagnostics snapshot. Full Tools Object Details shows the selected effective material description; material-instance edits, dependency inspection, refresh, and per-override or all-override reset are available through the typed C asset API, not yet as a complete in-panel authoring workflow.
