...
- In Scene Objects, use Create Native Rocket to create a nontrivial multi-part rocket directly through the Henka authoring mesh path. It becomes the selected native source with a manager-owned material instance and supports the same bounded component, topology, material, and project save/reload workflow; the current source is session-created and does not yet restore automatically in a fresh process.
