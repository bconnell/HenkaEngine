...
Fixed physics substeps commit atomically. If scratch allocation fails, the prior bodies, contacts, events, pair history, and linked scene transforms remain unchanged so the step can be retried after the allocation condition is resolved.
