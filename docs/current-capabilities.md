...
local-light/probe settings. v11 data retains direct-lighting and fog settings
while using default local-light/probe settings. Legacy documents without an
authored environment retain the runtime default; borrowed HDR textures and
renderer-owned captured GPU resources are not serialized.
...
- bounded homogeneous closed interior vertex-fan offset/cap replacement; the original selected vertex remains a valid loose vertex
...
For a homogeneous closed interior fan, it replaces the incident fan with an offset cap and preserves the original selected vertex as a valid loose vertex; it does not publish a non-manifold single-vertex side wall.
...
interior fans with mixed material or smoothing metadata also fail closed.
...
Vertex/Edge Extrude workflows beyond the bounded homogeneous closed-fan cap
...
- bounded static triangle-mesh colliders for sphere, upright-capsule, and
  axis-aligned-box contacts plus raycasts;
- contacts;
- impulses;
- friction;
- restitution;
...
behavior, and step offsets are not currently implemented. Contact-aware planar
sliding against blocking contacts, basic traversal across supported walkable
planes, bounded kinematic-platform motion inheritance, configurable
slope-aware grounding classification, and the accepted ground normal are
available. Mesh collision, constraints, and advanced simulation also remain
open.
