...
This keeps the current scene model lightweight while making object inspection and picking less sandbox-specific. Scene identities use opaque 64-bit generation-checked handles, not array indexes. Destroying an entity advances its slot generation before reuse, so a stale selection, action record, physics link, or authoring reference cannot silently target the replacement entity. Names, tags, material names, and interaction prompts are copied into bounded scene-owned storage, so callers may use temporary input buffers. Returned text pointers remain valid until the corresponding value is replaced or the entity is destroyed.
...
native BC1 RGB/RGBA and BC3 payloads are checked against independent GPU capabilities;
...
Materials now expose a bounded metallic-roughness subset: base color, metallic, roughness, tangent-space normal scale, occlusion strength, emissive color and strength, bounded clearcoat and optional sheen color and roughness, alpha mode, double-sided state, cast/receive-shadow intent, bounded volume attenuation controls, and runtime-authored subsurface amount/tint controls. Material assignment validates texture semantic usage and color space. Manager-owned glTF material definitions now create stack-owned instances with validated parameter overrides, revision-aware refresh after transactional reimport, semantic dependency inspection, transactional reset of one or all overrides, and a transactional bridge that applies the validated effective instance view to a scene entity; the definition remains the shared glTF material authority and no second JSON schema is introduced. glTF transmission scalar and volume-thickness textures are retained as manager-owned linear dependencies and modulate the corresponding bounded renderer responses. The OpenGL path uses GGX distribution, correlated Smith visibility, Schlick Fresnel, a dielectric F0, energy-conserving diffuse/specular separation, and bounded clearcoat and sheen lobes whose base-layer transmission is attenuated before the secondary response is added. When subsurface amount is nonzero, direct sun, moon, and supported local lights add an explicitly bounded three-lobe diffusion-profile approximation tinted by the shared subsurface color, with authored scalar or glTF linear thickness data widening the response and a geometric-normal derivative acting as a bounded curvature proxy at thin transitions. This is a local direct-light approximation, not true multi-scatter diffusion, a skin/wax profile system, or screen-space/ray-traced SSS. Scene environments can now borrow a validated linear HDR equirectangular texture for the background and material environment response while retaining the analytical gradient fallback; irradiance, prefiltered specular, and BRDF-LUT resources are derived transactionally when the GPU path is available. HDR target and shadow diagnostics report dimensions, generations, completeness, and bounded failure text. Generated UV spheres use nondegenerate fan caps, and mesh-upload tangent accumulation starts from zero so valid UV derivatives are not biased by fallback axes; degenerate UVs still receive finite orthogonal fallbacks. Model data can now carry a finite imported tangent frame and its handedness, which the upload path preserves after normal orthogonalization; legacy and procedural data continue to use generated tangents. Full MikkTSpace conformance remains future work. Blended entities always render after opaque and masked entities. Up to 4,096 blended entities use an allocation-free O(n log n) back-to-front sort with explicit depth/index tie-breaking; overflow falls back to deterministic entity order while preserving the opaque-first pass, and overflow count is exposed in diagnostics. Scenes support a bounded four-light point/spot list with normalized directions, inverse-square/range falloff, and spot cones in the GL 3.3 path; two fitted directional cascades, one deterministic 512² map for the first enabled spot, and one bounded 256² cubemap for the first enabled point provide bounded local shadowing. Distance fog is explicitly selected as linear, exponential, or exponential-squared, is disabled by default, and remains a distance effect. Volumetric fog is not implemented. Renderer diagnostics also report bounded distance-based LOD selections and configured-LOD fallback use; this is selection/culling foundation, not texture streaming or residency. Editor-authored material files and other advanced lobes remain future work.

The glTF scene instantiation path preserves the borrowed manager-owned material-definition identity on each created entity beside its effective material value. This keeps editor and dependency tooling connected to the shared asset authority without inferring identity from names or copied parameters. The normal engine frame path now refreshes direct scene bindings when a definition or owning glTF scene revision changes, so renderer consumers converge without requiring an Object Details visit; explicitly applied material instances are marked as overrides and remain untouched for their owning editor binding to refresh transactionally. The sandbox Object Details panel creates a bounded persistent instance for any selected imported scene entity with that identity; overrides are retained per entity, definition revision changes refresh the candidate and apply it transactionally, and Reimport routes by definition identity through the standalone material cache or owning glTF-scene transaction. Every shared instance parameter can be selected, scalar/vector values can be stepped, booleans and alpha mode can be changed, semantic texture overrides can be cleared or restored, dependencies and definition revision are visible, and reset preserves transactional failure behavior. Its texture rows cover the five core material slots plus imported transmission and volume-thickness texture slots and use the same semantic validation as the public instance API. Built-in procedural materials remain inspect-only; text-entry import, drag/drop, material-file authoring, and dedicated dependency-graph panels remain future work.

Runtime-created textures can join the same manager dependency graph through `henka_assets_adopt_runtime_texture`. The API validates a confined stable identity, live GPU payload, and configured residency budget before transferring ownership; duplicate identities are rejected and the resulting metadata explicitly reports that no source-file reload exists. This keeps generated HDR environments from becoming lifetime exceptions while preserving the shared glTF/material texture authority.

The HDR environment path now derives irradiance, prefiltered specular, and BRDF-LUT resources transactionally from the validated borrowed equirectangular texture; the analytical gradient remains the truthful fallback when derivation cannot complete. The source may be imported or manager-adopted at runtime, while the renderer never takes ownership of the borrowed scene pointer. The supported IBL material lookup caps roughness LOD at the generated 32x32 prefilter level so the reference path does not expose visibly under-resolved 4x4 or 2x2 mip structure. Optional local reflection probes own bounded 64x64 RGBA16F cubemaps in the OpenGL backend, and their roughness lookup is capped at the supported 16x16 prefilter level so high-roughness subjects do not expose under-resolved 8x8, 4x4, 2x2, or 1x1 mip structure. One changed probe is captured at a time from six deterministic camera directions with explicit per-face up vectors, using the camera's validated roll-aware look-at path, candidate faces are committed only after the complete capture succeeds, and probe sampling is disabled during capture to prevent recursion. Candidate allocation rejects latent OpenGL errors and restores the caller's active cube-texture binding; capture restores framebuffer, renderbuffer, viewport, clear-color, and active cube-texture state and rejects a face when the draw path reports a GL error. Enabled probes are ranked deterministically; when two captured volumes overlap the receiver, their specular and bounded diffuse contributions are blended with a bounded inverse-score weight. Box projection is independently applied per captured probe, while an uncaptured or invalid probe falls back to the shared IBL resources. The sandbox editor can visualize enabled probe volumes and their box-projection mode without changing scene data.
...
The sandbox's periodic HDR fixture is adopted by the asset manager under
`runtime/environment/studio`; the renderer still receives the same borrowed
linear equirectangular pointer, while manager cleanup and residency accounting
now cover the generated source as well as imported dependencies.
...
When a scene supplies a linear HDR equirectangular texture, the renderer derives bounded environment, irradiance, prefiltered-specular, and BRDF-LUT resources through one framebuffer-owned build transaction. The material shader samples those resources for diffuse and roughness-aware specular response; a failed derivation retains the source equirectangular path and reports IBL fallback diagnostics. The sandbox fixture uses a generated periodic source rather than a discontinuous low-resolution color table, and its explicit unlit grid remains on the line shader in Solid mode so environment or generic-surface lighting cannot create a viewport-center crease.

The Rendered presentation keeps one bounded display-space RGBA8 history texture, a transactionally replaced Scene View-sized depth-history texture/FBO, a Scene View-sized RG16F motion attachment, an R8 reactive attachment, and a sampled current depth attachment. The built-in material shader writes camera- and object-motion vectors from the current and previous view-projection matrices and marks transparent, transmissive, and emissive pixels as reactive. The tone-map pass reprojects history by motion against the retained previous-frame depth, rejects invalid or depth-inconsistent samples, reduces accumulation for reactive and high-motion pixels, clamps history to the current 3x3 tone-mapped neighborhood, and applies bounded neighborhood reconstruction sharpening. Rendered projection uses a bounded eight-sample Halton subpixel jitter sequence; history and the previous view-projection are invalidated when the temporal path becomes unavailable, the shading mode changes, the viewport is resized, or a large camera/projection discontinuity is detected. Diagnostics expose whether the temporal path is using its fallback, cumulative history resolves and fallback frames, the current invalidation/fallback reason and count, counted history-target allocation failures, and whether the previous history resource was retained after a failed replacement; a successful color/depth commit reports `history valid` so an old failure reason cannot masquerade as an active fault. Color, motion, reactive, current-depth, and previous-depth targets are replaced transactionally on resize, and history is invalidated until the first successful color/depth copy. Custom shaders without the optional motion contract and entities with no valid vector fall back to zero motion. The AO presentation now uses a bounded four-direction, two-sided, multi-step view-space horizon search with explicit radius, thickness, falloff, bias, and intensity controls; temporal AO history, denoise filtering, and production GTAO validation remain unfinished. The temporal path remains a bounded reconstruction foundation pending visual validation across camera cuts, resize, disocclusion, and moving-object cases; it is not a claim of complete production TAA or SSAO.

The Windows sandbox `--temporal-stress` mode provides an application-only
...
after the sequence; the command also prints history-target allocation-failure
and previous-resource-retention diagnostics, and returns failure if that
bounded check does not recover. This is deterministic runtime coverage for the
invalidation paths, not a claim that production TAA visual QA is closed.

The directional shadow path uses two fitted orthographic cascades around the active camera: a 24-unit near coverage and a 72-unit far coverage, each with a texel-stabilized center and bounded PCF. Manual PCF uses nearest depth fetches so blocker comparisons are not pre-interpolated, and out-of-projection spot taps use a lit border value. The receiver shader blends the near and far visibility over a bounded overlap around the split instead of hard-switching at one distance; this avoids a cascade-induced lighting crease on broad receivers. This stabilizes the map during sub-texel camera motion while retaining slope-aware bias and an adaptive near-cascade kernel that widens only near a receiver blocker to tighten directional shadow contacts. The first enabled spot light has a deterministic bounded 512² perspective depth map with a separate PCF path, and the first enabled point light has a bounded 256² six-face cubemap with linearized depth comparison. This is not a screen-space contact-ray pass.

The Windows sandbox `--material-stress` mode uses the existing shared glTF material definition and stack-owned instance APIs to exercise every supported scalar, vector, base-color, boolean, alpha-mode, transmission-texture, thickness-texture, and other semantic-texture override family on the live marker. It verifies invalid-edit retention, applies the effective scene material transactionally, refreshes the definition revision, resets overrides, and reapplies. A failure restores the previous instance and scene material. This is runtime API coverage for material instances, not persisted material-file authoring or a second material schema.

The sandbox now includes a bounded realism validation row covering rough and polished metal, painted clearcoat, plastic, stone-like roughness, fabric sheen, dry wood, wet/dry roughness variation, and a subsurface-tinted backscatter sphere under the same HDR environment, direct light, shadow, IBL, reflection-probe, and bloom paths. The stone, fabric, wood, and wet/dry samples use asset-manager-owned runtime macro, detail-normal, wood-grain, and metallic-roughness textures with their required semantic descriptors. A separate double-sided foliage card uses a bounded alpha-mask texture and participates in the normal shadow/material path. Rendered now carries authored per-pixel roughness from the material pass into the HDR target, allowing the bounded depth-derived screen-space reflection path to attenuate hits by the actual surface response before falling back to environment/probe/analytical lighting. glTF `KHR_materials_transmission` factor and `KHR_materials_volume` attenuation and linear thickness-texture controls now feed bounded thickness-aware direct-light and environment-based transmission/volume responses; runtime-authored subsurface controls feed the thickness-shaped three-lobe direct-light and back-facing environment approximation described above. Refraction, layered volume, true multi-scatter SSS, production glass, and volumetric fog remain explicit fallbacks or future renderer contracts.
The retained SSR implementation uses a fixed 16-step ray march plus four bounded local hit-refinement samples with signed depth-crossing validation, valid central depth-neighbor surface reconstruction, back-facing and edge-invalid hit rejection, bounded thickness, an 8-unit maximum distance, quadratic roughness attenuation, a roughness-aware five-tap resolve, edge confidence, smooth-material eligibility, and explicit miss fallback. Rough materials outside that eligibility bound use the filtered environment/probe response instead of unstable screen-space samples. This keeps the implementation testable without presenting unstable self-reflections as material response; it remains a screen-space approximation and does not claim planar or production hierarchical reflection coverage. Unsupported material categories continue to use the environment/probe fallback.
...
The current sandbox layers safe panel paging and bounded persisted native detached panel placement on top of that docked layout so scene-first modes remain readable while workspace movement is evaluated.
...
Henka now has a small multi-window platform foundation: secondary OS-level tool windows receive collision-safe engine identifiers, validate native window identifiers before admission, route close/focus/resize/move/pointer events separately from the main viewport input path, and can present their own UI-only OpenGL surface. Focus loss synthesizes releases for held main-window keys and mouse buttons, releases a held detached-window left button, and clears transient motion. Detached-window press, release, resize, and position state is reset or updated per frame while close and resize diagnostics remain aggregated for the full event poll. VSync and mouse capture report failures instead of recording partially applied state, platform queries clear caller outputs on failure, and diagnostics report the latest verified multi-window capability. Success is not assumed. Main and detached-window OpenGL context transitions are checked before resource deletion, frame abort reports context-restoration failure without falsely clearing logical frame ownership, and scene drawing binds either the current entity texture or texture zero for every entity so stale texture state cannot leak across objects. The sandbox uses this for `Native Panel Test` and for detached production-panel surfaces that render matching panel controls with routed release-confirm input, an explicit dock-return bar, bounded saved placement, and title-bar drag-back recognition. Detachable Scene View remains future work.
...
This is a small inspection layer, not a content browser or hot-reload pipeline. Source and display names are cache-owned strings, not borrowed caller buffers. Shader, texture, and mesh cache growth is bounded and checked before allocation, asset paths are bounded, and scene entity storage also uses bounded checked growth.
...
The sandbox workspace now tracks dock zones, allowed dock masks, last valid docks, transient header dragging, native detached-window handles and bounded positions, and dock splitter interactions in session state. Detached production-panel surfaces and `Native Panel Test` use separate-window rendering without routing pointer input into Scene View. Closing a detached production panel safely returns it to its last dock, valid saved virtual-screen positions are reapplied on the next detach, and a focused native-window move into the main-window envelope is recognized as a title-bar drag-back request. The Scene View remains the main viewport and remains non-detachable.
...
The Scene View owns an explicit shading mode. It does not rely on a global polygon toggle. Wireframe draws neutral geometry edges without texture sampling. Solid draws neutral filled surfaces under a neutral editor surface policy. Material Preview uses the same bounded Cook–Torrance material evaluation and Scene View-sized linear HDR-to-display presentation as Rendered, with deterministic editor lighting instead of scene lighting. Both HDR modes use validated scene-owned environment controls for visible surroundings and diffuse/specular response; Rendered derives transactional IBL resources from the same environment texture only while the scene is in HDRI mode. The shared environment descriptor also supports gradient and bounded procedural-atmosphere modes, deterministic sun direction, headless-safe time-of-day advancement, bounded procedural moon-disc/star presentation, a shadowless bounded moon direct-light contribution for Rendered PBR materials, and four shared starting presets; the rendered sky and material environment sampling consume those fields without introducing a separate sky authority. Switching away from HDRI transactionally discards derived IBL and uses an explicit analytical environment fallback, so procedural or gradient PBR shading cannot retain stale HDR resources. Sandbox Environment settings persist and reload the complete bounded scalar/celestial descriptor through the existing transactional settings file, and the stress path exercises an in-memory save/load round trip before rendering; the Settings utility applies presets through the same scene setter, while the generated studio environment source is recreated through the package-owned runtime path. Dedicated moon shadow maps, arbitrary HDRI/cubemap identity persistence, procedural IBL convolution, editor environment asset authoring, transactional environment hot reload, and weather/cloud layers remain future work. Rendered alone applies scene light policy, exposure, tone mapping, bloom, bounded temporal accumulation, depth-neighborhood AO presentation, and the directional shadow path; Material Preview intentionally omits those scene-dependent post-processing effects. Unlit materials bypass lighting, reserved procedural materials are rejected until a real shader model exists, and helper overlays retain their own materials. Mode changes do not rewrite scene materials, and the renderer restores a filled polygon baseline before UI and detached-window presentation.
...
Material instances layer validated scalar, vector, alpha-mode, and semantic-texture overrides over the shared glTF material definition. Effective dependency inspection reports the borrowed texture slots after those overrides, including the imported transmission scalar and volume-thickness slots, while revision-aware refresh preserves explicit instance choices across transactional definition reimport.
