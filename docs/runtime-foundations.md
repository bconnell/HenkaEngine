...
The renderer diagnostics also report the last scene draw-call count, visible-entity count, conservative frustum-cull count, and transparent-sort overflow count. These are bounded per-frame counters, not a claim of GPU timing or complete draw-cost attribution. They also expose tracked mesh/texture upload bytes, persistent HDR/shadow target bytes, and resource counts as engine-side lower-bound estimates; they are not driver-reported physical VRAM.
