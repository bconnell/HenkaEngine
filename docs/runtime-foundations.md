...
Materials now expose a bounded PBR-lite subset: base color, metallic, roughness, tangent-space normal scale, emissive color and strength, alpha mode, double-sided state, and cast/receive-shadow intent. The current renderer consumes normal, metallic-roughness, and emissive maps when supplied. It is still a forward renderer without image-based lighting or a general material asset parser.
