...
The Rendered presentation keeps one bounded display-space RGBA8 history texture and a Scene View-sized RG16F motion attachment. The built-in material shader writes camera-motion vectors from the current and previous view-projection matrices, and the tone-map pass reprojects history by that vector before applying a small fixed blend. History and velocity targets are replaced transactionally on resize, and history is invalidated until the first successful copy. Custom shaders without the optional motion contract and entities with no valid vector fall back to zero motion. Object-motion vectors, disocclusion rejection, neighborhood clamping, jitter, and full production TAA remain unfinished; this is a camera-motion-compensated temporal foundation, not a claim of complete TAA.
