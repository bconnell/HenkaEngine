...
The Scene View owns an explicit shading mode. It does not rely on a global polygon toggle. Wireframe draws neutral geometry edges without texture sampling. Solid draws neutral filled surfaces under a neutral editor surface policy. Material Preview uses the same bounded Cook–Torrance material evaluation and Scene View-sized linear HDR-to-display presentation as Rendered, with deterministic editor lighting instead of scene lighting. Both HDR modes use validated scene-owned environment controls for visible surroundings and diffuse/specular response; Rendered derives transactional IBL resources from the same environment texture only while the scene is in HDRI mode. The shared environment descriptor also supports gradient and bounded procedural-atmosphere modes, deterministic sun direction, headless-safe time-of-day advancement, and bounded procedural moon-disc/star presentation; the rendered sky and material environment sampling consume those fields without introducing a separate sky authority. Switching away from HDRI transactionally discards derived IBL and uses an explicit analytical environment fallback, so procedural or gradient PBR shading cannot retain stale HDR resources. Sandbox Environment settings persist and reload the complete bounded scalar/celestial descriptor through the existing transactional settings file, and the stress path exercises an in-memory save/load round trip before rendering; the generated studio environment source is recreated through the package-owned runtime path. Moon direct-lighting, arbitrary HDRI/cubemap identity persistence, procedural IBL convolution, editor environment asset authoring, transactional environment hot reload, and weather/cloud layers remain future work. Rendered alone applies scene light policy, exposure, tone mapping, bloom, bounded temporal accumulation, depth-neighborhood AO presentation, and the directional shadow path; Material Preview intentionally omits those scene-dependent post-processing effects. Unlit materials bypass lighting, reserved procedural materials are rejected until a real shader model exists, and helper overlays retain their own materials. Mode changes do not rewrite scene materials, and the renderer restores a filled polygon baseline before UI and detached-window presentation.
