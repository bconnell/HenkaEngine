...
The Rendered presentation also keeps one bounded display-space RGBA8 history texture. It is replaced transactionally on Scene View resize, invalidated until the first successful copy, and blended by a small fixed accumulation factor in the tone-map pass. This is a deliberately simple temporal accumulation fallback rather than motion-compensated TAA: it provides stable presentation for static or slowly changing views, while the renderer truthfully exposes no temporal history when allocation or copy is unavailable.
...
The renderer diagnostics also report the last scene draw-call count, visible-entity count, conservative frustum-cull count, and transparent-sort overflow count. These are bounded per-frame counters, not a claim of GPU timing or complete draw-cost attribution. They also expose categorized mesh, texture, and render-target bytes, current and peak logical GPU estimates, resource counts, and an explicit arithmetic-overflow state; they are not driver-reported physical VRAM.
