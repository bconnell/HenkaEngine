...
The renderer diagnostics also report the last scene draw-call count, visible-entity count, conservative frustum-cull count, bounded compatible-instance draw/entity counts, transparent-sort overflow count, CPU scene-render duration, and optional non-stalling OpenGL timer-query duration. Missing timer-query or instancing support is reported through ordinary draw/fallback behavior rather than synthesized capability. These are bounded measurements, not complete draw-cost attribution. They also expose categorized mesh, texture, and render-target bytes, current and peak logical GPU estimates, resource counts, and an explicit arithmetic-overflow state; they are not driver-reported physical VRAM.
