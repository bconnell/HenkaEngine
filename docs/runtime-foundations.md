...
after the sequence; the command returns failure if that bounded check does not
recover. This is deterministic runtime coverage for the invalidation paths, not
a claim that production TAA visual QA is closed.
