...
The Windows sandbox `--material-stress` mode uses the existing shared glTF material definition and stack-owned instance APIs to apply typed scalar, boolean, and alpha-mode overrides to the live marker, verify the effective scene material, refresh the definition revision, reset overrides, and reapply transactionally. A failure restores the previous instance and scene material. This is runtime API coverage for material instances, not persisted material-file authoring or a second material schema.
