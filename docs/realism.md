...
shader lookup. The earlier localized IBL reflection-knot regression remains
protected by executable negative controls, but exact-head normal-color,
diffuse-only, specular-only, simple-environment, packaged reference, and
renderer-family checks do not reproduce it.

The separate nips/spot appearance is routed to Modeling and Authoring. It is
not an active renderer-owned defect on the current exact head. Broader IBL
authoring, probe-grid workflows, and other renderer backends remain future
expansion.
...
Reference detail maps are generated at 128x128:
...
The generated maps use tileable multi-scale value noise with bounded mid-scale octaves that avoid periodic stripe signals and broad block-like bands. Capture readiness reads runtime texture dimensions and fails below the required minimum.
...
The current checker also includes a synthetic known-bad negative control for the localized reflection-knot defect. Passing that regression does not establish universal IBL accuracy across arbitrary HDR environments, materials, or hardware.
