...
- [x] Use world-space terrain UVs with per-layer meter-scale tiling and deterministic factor fallback when a texture is absent; the Sandbox smoke now asserts all four layers carry bounded Sandbox-owned procedural base-color textures.
