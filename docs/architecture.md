...
Directional shadow cascade selection uses interpolated forward view-space depth rather than radial camera distance, so the bounded near/far overlap remains stable when the camera looks across a wide scene. This improves cascade presentation without changing the fitted map ownership or the documented bounded-shadow fallback policy.
