...
| Scene hierarchy and parenting | Runtime parent/unparent/reparent, keep-local/keep-world transforms, cycle rejection, persisted parent IDs, and bounded prefab snapshot hierarchy | Scene hierarchy APIs and `scene_document.c`; `test_sandbox3d_game_authoring_hierarchy.c` and related identity tests | Runtime and Scene Document foundations are connected; editor outliner/history and all-subsystem hierarchy authoring are not complete | **Foundation** | Full editor hierarchy, cross-subsystem transform propagation, durable project manifests, and history remain open. |
