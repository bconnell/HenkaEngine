...
| Undo/redo and history | Shared bounded authoring history, material history, terrain edit history, and workspace layout history for the implemented operations | `authoring_mesh.c`, terrain history, workspace persistence; authoring, material, terrain, and editor tests | Supported operations commit through transactional source/history boundaries; package-level authoring history is not complete | **Foundation** | Cross-subsystem history, broader editor coverage, and complete project-level history remain open. |
| Scene hierarchy and parenting | Runtime parent/unparent/reparent, keep-local/keep-world transforms, cycle rejection, persisted parent IDs, bounded prefab snapshot hierarchy, parent-first identity-safe Scene Objects projection, and explicit Object Details hierarchy controls | Scene hierarchy APIs and `scene_document.c`; `examples/sandbox3d/scene_hierarchy_projection.c`, `game_authoring.c`, and `main.c`; hierarchy, projection, and Game Authoring tests | Runtime and Scene Document foundations are connected; the Scene Objects panel and Object Details hierarchy group project and mutate the canonical parent relationship without creating a second hierarchy authority | **In Progress** | Cross-subsystem transform propagation, durable project manifests, shared hierarchy history, and broader packaged visual proof remain open. |
