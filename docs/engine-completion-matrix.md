...
| Scene hierarchy and parenting | Runtime parent/unparent/reparent, keep-local/keep-world transforms, cycle rejection, persisted parent IDs, bounded prefab snapshot hierarchy, and a parent-first identity-safe Scene Objects projection | Scene hierarchy APIs and `scene_document.c`; `examples/sandbox3d/scene_hierarchy_projection.c`; `test_sandbox3d_game_authoring_hierarchy.c`, projection, and related identity tests | Runtime and Scene Document foundations are connected; the Scene Objects panel projects canonical parent/child order and depth without creating a second hierarchy authority | **Foundation** | Direct editor reparent controls, cross-subsystem transform propagation, durable project manifests, and history remain open. |
