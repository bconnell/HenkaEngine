...
relative asset paths from the executable base directory rather than the process
working directory. The Windows external-template validator launches a dedicated
smoke mode from a separate empty working directory and loads the packaged
`assets/audio/henka_audio_fixture.wav` through the public asset manager. This
keeps the relocatable package-root contract executable instead of relying only
on documentation.
