...
boundary, the manager-owned asset metadata path, real Scene Document emitter
and listener configuration, and the Sandbox Play lifecycle. Those integrations
must preserve the renderer-free dedicated-server path and add real imported or
authored-object coverage rather than test-only entities.
