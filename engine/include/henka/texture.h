...
/*
 * Creates a texture from an image file. The caller owns the texture and must
 * release it with henka_texture_destroy.
 */
henka_result henka_texture_create_from_file(henka_engine* engine, const char* path, henka_texture** out_texture);

/*
 * Creates a texture from RGBA8 pixel data. The caller owns the texture and
 * must release it with henka_texture_destroy.
 */
