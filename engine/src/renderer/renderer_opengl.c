...
static void henka_opengl_delete_hdr_target(henka_opengl_renderer_state* state)
{
    ...
    {
        target_bytes = (uint64_t)state->hdr_width * (uint64_t)state->hdr_height * 20U;
    ...
    int height)
{
    ...
    if (framebuffer == 0U || color_texture == 0U || motion_texture == 0U || depth_buffer == 0U)
    {
    ...
    g_gl.FramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT1, GL_TEXTURE_2D, motion_texture, 0);
    {
        ...
        if (g_gl.DrawBuffers != NULL)
            g_gl.DrawBuffers(2, draw_buffers);
    }
    ...
    {
        uint64_t target_bytes = (uint64_t)width * (uint64_t)height * 20U;
        henka_opengl_memory_add_category(
            state,
            &state->tracked_render_target_bytes,
            target_bytes);
    ...
    henka_opengl_renderer_state* state)
{
    ...
    static const char* tone_fragment =
        ...
        "void main(){ vec3 color = texture(hdrTexture, uv).rgb; if (useBloom) color += texture(bloomTexture, uv).rgb * max(bloomStrength, 0.0); color *= exp2(exposure); color = pow(aces(max(color, vec3(0.0))), vec3(1.0/2.2)); if (useAmbientOcclusion) { float depth = texture(depthTexture, uv).r; if (depth < 0.9999) { vec2 texel = 1.0 / vec2(textureSize(depthTexture, 0)); float occluded = 0.0; const vec2 offsets[8] = vec2[8](vec2(-2.0, 0.0), vec2(2.0, 0.0), vec2(0.0, -2.0), vec2(0.0, 2.0), vec2(-1.0, -1.0), vec2(1.0, -1.0), vec2(-1.0, 1.0), vec2(1.0, 1.0)); for (int i = 0; i < 8; ++i) { float neighbor = texture(depthTexture, clamp(uv + offsets[i] * texel, vec2(0.001), vec2(0.999))).r; occluded += neighbor < depth - 0.002 ? 1.0 : 0.0; } color *= mix(1.0, 0.68, occluded / 8.0); } } if (useTemporalHistory) { vec2 historyUv = uv; if (useMotionVectors) historyUv = clamp(uv - texture(motionTexture, uv).rg, vec2(0.001), vec2(0.999)); float currentDepth = texture(depthTexture, uv).r; float reprojectedDepth = texture(depthTexture, historyUv).r; float historyWeight = abs(currentDepth - reprojectedDepth) > 0.02 ? 0.0 : clamp(temporalBlend, 0.0, 0.25); color = mix(color, texture(historyTexture, historyUv).rgb, historyWeight); } outColor = vec4(color, 1.0); }\n";
