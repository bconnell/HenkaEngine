...
"
        ...
        "vec3 screenIndirectDiffuse(vec2 at, vec2 texel, float depth, vec3 currentHdr){ if(depth>=0.9999) return vec3(0.0); vec3 position=viewPosition(at,depth); vec2 xUv=clamp(at+vec2(texel.x,0.0),vec2(0.001),vec2(0.999)); vec2 yUv=clamp(at+vec2(0.0,texel.y),vec2(0.001),vec2(0.999)); vec3 px=viewPosition(xUv,texture(depthTexture,xUv).r); vec3 py=viewPosition(yUv,texture(depthTexture,yUv).r); vec3 normal=normalize(cross(py-position,px-position)); const vec2 directions[8]=vec2[8](vec2(1.0,0.0),vec2(-1.0,0.0),vec2(0.0,1.0),vec2(0.0,-1.0),vec2(0.7071,0.7071),vec2(-0.7071,0.7071),vec2(0.7071,-0.7071),vec2(-0.7071,-0.7071)); vec3 indirect=vec3(0.0); float totalWeight=0.0; for(int directionIndex=0;directionIndex<8;++directionIndex){ for(int step=1;step<=2;++step){ float pixelDistance=3.0+6.0*float(step); vec2 sampleUv=clamp(at+directions[directionIndex]*texel*pixelDistance,vec2(0.002),vec2(0.998)); float sampleDepth=texture(depthTexture,sampleUv).r; if(sampleDepth>=0.9999) continue; vec3 samplePosition=viewPosition(sampleUv,sampleDepth); vec3 delta=samplePosition-position; float distanceToSample=length(delta); if(distanceToSample<=0.001||distanceToSample>max(ssgiRadius,0.01)) continue; vec3 incomingDirection=delta/distanceToSample; float receiverCosine=max(dot(normal,incomingDirection),0.0); if(receiverCosine<=0.0001) continue; float thicknessWeight=1.0-clamp(abs(delta.z)/max(ssgiThickness,0.001),0.0,1.0); float distanceWeight=1.0-clamp(distanceToSample/max(ssgiRadius,0.01),0.0,1.0); float weight=receiverCosine*thicknessWeight*distanceWeight; vec3 sampleHdr=clamp(texture(hdrTexture,sampleUv).rgb,vec3(0.0),vec3(max(ssgiMaxRadiance,0.0))); indirect+=sampleHdr*weight; totalWeight+=weight; } } if(totalWeight<=0.0001) return vec3(0.0); vec3 bounced=indirect/totalWeight; float currentLuma=dot(max(currentHdr,vec3(0.0)),vec3(0.2126,0.7152,0.0722)); float limiter=1.0/(1.0+currentLuma*0.1); return bounced*clamp(ssgiIntensity,0.0,1.0)*limiter; }\
"
