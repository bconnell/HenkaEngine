...
"
        ...
        "vec3 screenReflection(vec2 at, vec2 texel, float depth, vec3 base){ vec3 position; vec3 normal; if(!screenReflectionSurface(at,texel,depth,position,normal)) return base; vec3 direction=normalize(reflect(normalize(position),normal)); vec3 reflected=base; float confidence=0.0; float hitDistance=0.0; float maxDistance=clamp(ssrMaxDistance,0.25,32.0); for(int step=1;step<=16;++step){ float distanceAlongRay=maxDistance*(float(step)/16.0); vec3 rayPosition=position+direction*distanceAlongRay; vec4 clip=projection*vec4(rayPosition,1.0); if(clip.w<=0.0) break; vec2 sampleUv=clip.xy/clip.w*0.5+0.5; if(sampleUv.x<0.002||sampleUv.x>0.998||sampleUv.y<0.002||sampleUv.y>0.998) break; float sceneDepth=texture(depthTexture,sampleUv).r; if(sceneDepth>=0.9999) continue; vec3 scenePosition=viewPosition(sampleUv,sceneDepth); float depthDelta=abs(scenePosition.z-rayPosition.z); if(depthDelta<=max(ssrThickness,0.001)){ vec3 hitPosition; vec3 hitNormal; if(!screenReflectionSurface(sampleUv,texel,sceneDepth,hitPosition,hitNormal) || dot(hitNormal,-direction)<=0.05) continue; float materialRoughness=clamp(texture(roughnessTexture,at).r,0.045,1.0); reflected=filteredScreenReflection(sampleUv,texel,materialRoughness); hitDistance=distanceAlongRay; float edgeDistance=min(min(sampleUv.x,1.0-sampleUv.x),min(sampleUv.y,1.0-sampleUv.y)); float edgeConfidence=clamp(edgeDistance/max(ssrEdgeFade,0.001),0.0,1.0); float roughnessConfidence=(1.0-materialRoughness)*(1.0-materialRoughness); confidence=0.45*edgeConfidence*roughnessConfidence*(1.0-clamp(hitDistance/maxDistance,0.0,1.0)*0.5); break; } } return mix(base,reflected,clamp(confidence,0.0,0.45)); }\n"
        "vec3 screenReflectionRefined(vec2 at, vec2 texel, float depth, vec3 base){ vec3 position; vec3 normal; if(!screenReflectionSurface(at,texel,depth,position,normal)) return base; vec3 direction=normalize(reflect(normalize(position),normal)); vec3 reflected=base; float confidence=0.0; float hitDistance=0.0; float maxDistance=clamp(ssrMaxDistance,0.25,32.0); float previousDistance=0.0; bool previousValid=false; for(int step=1;step<=16;++step){ float distanceAlongRay=maxDistance*(float(step)/16.0); vec3 rayPosition=position+direction*distanceAlongRay; vec4 clip=projection*vec4(rayPosition,1.0); if(clip.w<=0.0) break; vec2 sampleUv=clip.xy/clip.w*0.5+0.5; if(sampleUv.x<0.002||sampleUv.x>0.998||sampleUv.y<0.002||sampleUv.y>0.998) break; float sceneDepth=texture(depthTexture,sampleUv).r; if(sceneDepth>=0.9999){ previousValid=false; continue; } vec3 scenePosition=viewPosition(sampleUv,sceneDepth); float thickness=max(ssrThickness,0.001); float depthDelta=scenePosition.z-rayPosition.z; if(depthDelta>=0.0 && depthDelta<=thickness){ vec2 bestUv=sampleUv; float bestDelta=depthDelta; float bestDistance=distanceAlongRay; float low=previousValid?previousDistance:max(0.0,distanceAlongRay-maxDistance/16.0); float high=distanceAlongRay; for(int refine=0;refine<4;++refine){ float refinedDistance=(low+high)*0.5; vec3 refinedPosition=position+direction*refinedDistance; vec4 refinedClip=projection*vec4(refinedPosition,1.0); if(refinedClip.w<=0.0){ low=refinedDistance; continue; } vec2 refinedUv=refinedClip.xy/refinedClip.w*0.5+0.5; if(refinedUv.x<0.002||refinedUv.x>0.998||refinedUv.y<0.002||refinedUv.y>0.998){ low=refinedDistance; continue; } float refinedDepth=texture(depthTexture,refinedUv).r; if(refinedDepth>=0.9999){ low=refinedDistance; continue; } float refinedDelta=abs(viewPosition(refinedUv,refinedDepth).z-refinedPosition.z); if(refinedDelta<bestDelta){ bestDelta=refinedDelta; bestUv=refinedUv; bestDistance=refinedDistance; } if(refinedDelta<=thickness) high=refinedDistance; else low=refinedDistance; } float bestDepth=texture(depthTexture,bestUv).r; vec3 hitPosition; vec3 hitNormal; if(!screenReflectionSurface(bestUv,texel,bestDepth,hitPosition,hitNormal) || dot(hitNormal,-direction)<=0.05){ previousValid=false; continue; } reflected=filteredScreenReflection(bestUv,texel,clamp(ssrRoughness,0.0,1.0)); hitDistance=bestDistance; float edgeDistance=min(min(bestUv.x,1.0-bestUv.x),min(bestUv.y,1.0-bestUv.y)); float edgeConfidence=clamp(edgeDistance/max(ssrEdgeFade,0.001),0.0,1.0); float roughnessValue=clamp(ssrRoughness,0.0,1.0); float roughnessConfidence=(1.0-roughnessValue)*(1.0-roughnessValue); float fresnelConfidence=schlickFresnel(max(dot(normal,normalize(-position)),0.0)); confidence=0.45*edgeConfidence*roughnessConfidence*fresnelConfidence*(1.0-clamp(hitDistance/maxDistance,0.0,1.0)*0.5); break; } previousDistance=distanceAlongRay; previousValid=true; } return mix(base,reflected,clamp(confidence,0.0,0.45)); }\n"
