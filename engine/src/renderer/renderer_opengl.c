    ...
    henka_opengl_shader_data* out_shader_data)
{
    ...
    static const char* material_uniforms[] =
    {
        ...
        "emissiveTexture", "thicknessTexture",
...
static void henka_opengl_delete_hdr_target(henka_opengl_renderer_state* state)
{
    ...
    {
        target_bytes = (uint64_t)state->hdr_width * (uint64_t)state->hdr_height * 22U;
    ...
    if (state->hdr_roughness_texture != 0U)
    {
        glDeleteTextures(1, &state->hdr_roughness_texture);
    }
    ...
    state->hdr_roughness_texture = 0U;
    state->hdr_framebuffer = 0U;
    state->hdr_width = 0;
    state->hdr_height = 0;
    state->hdr_framebuffer_complete = false;
}
...
static uint64_t henka_opengl_reflection_probe_storage_bytes(void)
{
    ...
    for (mip = 0; mip < HENKA_REFLECTION_PROBE_PREFILTER_LEVELS; ++mip)
    {
        int resolution = HENKA_REFLECTION_PROBE_RESOLUTION >> mip;
        if (resolution < 1)
            resolution = 1;
        bytes += (uint64_t)resolution * (uint64_t)resolution * 6U * 8U;
    }
    return bytes;
}
    ...
    for (index = 0U; index < HENKA_SCENE_MAX_REFLECTION_PROBES; ++index)
    {
        if (state->reflection_probe_cubes[index] != 0U)
        {
            ...
            henka_opengl_memory_remove_category(
                state,
                &state->tracked_render_target_bytes,
                henka_opengl_reflection_probe_storage_bytes());
        }
    ...
    int height)
{
    ...
    if (framebuffer == 0U || color_texture == 0U || motion_texture == 0U ||
        reactive_texture == 0U || roughness_texture == 0U || depth_buffer == 0U)
    {
    ...
    g_gl.FramebufferTexture2D(GL_FRAMEBUFFER, GL_COLOR_ATTACHMENT3, GL_TEXTURE_2D, roughness_texture, 0);
    {
        ...
        if (g_gl.DrawBuffers != NULL)
            g_gl.DrawBuffers(4, draw_buffers);
    }
    ...
    state->hdr_roughness_texture = roughness_texture;
    ...
    {
        uint64_t target_bytes = (uint64_t)width * (uint64_t)height * 22U;
        henka_opengl_memory_add_category(
            state,
            &state->tracked_render_target_bytes,
            target_bytes);
    ...
    int resolution)
{
    ...
    g_gl.BindFramebuffer(GL_FRAMEBUFFER, framebuffer);
    glBindTexture(GL_TEXTURE_2D, depth_texture);
    /* The receiver shader performs its own PCF comparisons. Nearest depth
     * fetches keep those comparisons deterministic instead of interpolating
     * unrelated blocker depths before the comparison. */
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
    glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
...
static henka_result henka_opengl_create_render_programs(
    henka_opengl_renderer_state* state)
{
    static const char* tone_uniforms[] = {"hdrTexture", "bloomTexture", "historyTexture", "historyDepthTexture", "motionTexture", "reactiveTexture", "depthTexture", "roughnessTexture", "projection", "exposure", "useBloom", "bloomStrength", "useTemporalHistory", "useMotionVectors", "useReactiveMask", "useAmbientOcclusion", "aoRadius", "aoThickness", "aoFalloff", "aoBias", "aoIntensity", "useScreenSpaceIndirect", "ssgiRadius", "ssgiThickness", "ssgiIntensity", "ssgiMaxRadiance", "ssrThickness", "ssrMaxDistance", "ssrEdgeFade", "temporalBlend", "sharpenStrength", "useRenderedGrade", "useScreenSpaceReflections"};
...
"
        ...
        "float ssgiSourceFacing(vec2 at, vec2 texel, vec3 samplePosition, vec3 incomingDirection){ vec2 xUv=clamp(at+vec2(texel.x,0.0),vec2(0.002),vec2(0.998)); vec2 yUv=clamp(at+vec2(0.0,texel.y),vec2(0.002),vec2(0.998)); float xDepth=texture(depthTexture,xUv).r; float yDepth=texture(depthTexture,yUv).r; if(xDepth>=0.9999||yDepth>=0.9999) return 0.0; vec3 xPosition=viewPosition(xUv,xDepth); vec3 yPosition=viewPosition(yUv,yDepth); vec3 sourceNormal=normalize(cross(yPosition-samplePosition,xPosition-samplePosition)); float sourceCosine=max(dot(sourceNormal,-incomingDirection),0.0); return smoothstep(0.05,0.65,sourceCosine); }\n"
        "vec3 screenIndirectDiffuse(vec2 at, vec2 texel, float depth, vec3 currentHdr){ if(depth>=0.9999) return vec3(0.0); vec3 position=viewPosition(at,depth); vec2 xUv=clamp(at+vec2(texel.x,0.0),vec2(0.001),vec2(0.999)); vec2 nxUv=clamp(at-vec2(texel.x,0.0),vec2(0.001),vec2(0.999)); vec2 yUv=clamp(at+vec2(0.0,texel.y),vec2(0.001),vec2(0.999)); vec2 nyUv=clamp(at-vec2(0.0,texel.y),vec2(0.001),vec2(0.999)); float xDepth=texture(depthTexture,xUv).r; float nxDepth=texture(depthTexture,nxUv).r; float yDepth=texture(depthTexture,yUv).r; float nyDepth=texture(depthTexture,nyUv).r; if(xDepth>=0.9999||nxDepth>=0.9999||yDepth>=0.9999||nyDepth>=0.9999) return vec3(0.0); vec2 x2Uv=clamp(at+vec2(texel.x*2.0,0.0),vec2(0.001),vec2(0.999)); vec2 nx2Uv=clamp(at-vec2(texel.x*2.0,0.0),vec2(0.001),vec2(0.999)); vec2 y2Uv=clamp(at+vec2(0.0,texel.y*2.0),vec2(0.001),vec2(0.999)); vec2 ny2Uv=clamp(at-vec2(0.0,texel.y*2.0),vec2(0.001),vec2(0.999)); float x2Depth=texture(depthTexture,x2Uv).r; float nx2Depth=texture(depthTexture,nx2Uv).r; float y2Depth=texture(depthTexture,y2Uv).r; float ny2Depth=texture(depthTexture,ny2Uv).r; float receiverDepthDelta=max(max(max(abs(xDepth-depth),abs(nxDepth-depth)),max(abs(yDepth-depth),abs(nyDepth-depth))),max(max(abs(x2Depth-depth),abs(nx2Depth-depth)),max(abs(y2Depth-depth),abs(ny2Depth-depth)))); float receiverEdgeConfidence=1.0-clamp(receiverDepthDelta/0.025,0.0,1.0); receiverEdgeConfidence*=receiverEdgeConfidence; if(receiverEdgeConfidence<=0.05) return vec3(0.0); vec3 px=viewPosition(xUv,xDepth); vec3 nx=viewPosition(nxUv,nxDepth); vec3 py=viewPosition(yUv,yDepth); vec3 ny=viewPosition(nyUv,nyDepth); vec3 normal=normalize(cross(py-ny,px-nx)); const vec2 directions[8]=vec2[8](vec2(1.0,0.0),vec2(-1.0,0.0),vec2(0.0,1.0),vec2(0.0,-1.0),vec2(0.7071,0.7071),vec2(-0.7071,0.7071),vec2(0.7071,-0.7071),vec2(-0.7071,-0.7071)); vec3 indirect=vec3(0.0); float totalWeight=0.0; for(int directionIndex=0;directionIndex<8;++directionIndex){ for(int step=1;step<=2;++step){ float pixelDistance=3.0+6.0*float(step); vec2 sampleUv=clamp(at+directions[directionIndex]*texel*pixelDistance,vec2(0.002),vec2(0.998)); float sampleDepth=texture(depthTexture,sampleUv).r; if(sampleDepth>=0.9999) continue; vec3 samplePosition=viewPosition(sampleUv,sampleDepth); vec3 delta=samplePosition-position; float distanceToSample=length(delta); if(distanceToSample<=0.001||distanceToSample>max(ssgiRadius,0.01)) continue; vec3 incomingDirection=delta/distanceToSample; float receiverCosine=max(dot(normal,incomingDirection),0.0); if(receiverCosine<=0.0001) continue; float thicknessWeight=1.0-clamp(abs(delta.z)/max(ssgiThickness,0.001),0.0,1.0); float distanceWeight=1.0-clamp(distanceToSample/max(ssgiRadius,0.01),0.0,1.0); float sourceFacing=ssgiSourceFacing(sampleUv,texel,samplePosition,incomingDirection); float weight=receiverCosine*thicknessWeight*distanceWeight*sourceFacing; vec3 sampleHdr=texture(hdrTexture,sampleUv).rgb*0.40; sampleHdr+=texture(hdrTexture,clamp(sampleUv+vec2(texel.x,0.0)*1.5,vec2(0.002),vec2(0.998))).rgb*0.15; sampleHdr+=texture(hdrTexture,clamp(sampleUv-vec2(texel.x,0.0)*1.5,vec2(0.002),vec2(0.998))).rgb*0.15; sampleHdr+=texture(hdrTexture,clamp(sampleUv+vec2(0.0,texel.y)*1.5,vec2(0.002),vec2(0.998))).rgb*0.15; sampleHdr+=texture(hdrTexture,clamp(sampleUv-vec2(0.0,texel.y)*1.5,vec2(0.002),vec2(0.998))).rgb*0.15; sampleHdr=clamp(sampleHdr,vec3(0.0),vec3(max(ssgiMaxRadiance,0.0))); indirect+=sampleHdr*weight; totalWeight+=weight; } } if(totalWeight<=0.0001) return vec3(0.0); vec3 bounced=indirect/totalWeight; float currentLuma=dot(max(currentHdr,vec3(0.0)),vec3(0.2126,0.7152,0.0722)); float limiter=1.0/(1.0+currentLuma*0.1); return bounced*clamp(ssgiIntensity,0.0,1.0)*limiter*receiverEdgeConfidence; }\
"
        "\n"
        "float schlickFresnel(float cosine){ return 0.04 + 0.96 * pow(1.0 - clamp(cosine,0.0,1.0),5.0); }\n"
        "bool screenReflectionSurface(vec2 at, vec2 texel, float depth, out vec3 position, out vec3 normal){ if(depth>=0.9999) return false; vec2 xUv=clamp(at+vec2(texel.x,0.0),vec2(0.002),vec2(0.998)); vec2 nxUv=clamp(at-vec2(texel.x,0.0),vec2(0.002),vec2(0.998)); vec2 yUv=clamp(at+vec2(0.0,texel.y),vec2(0.002),vec2(0.998)); vec2 nyUv=clamp(at-vec2(0.0,texel.y),vec2(0.002),vec2(0.998)); float xDepth=texture(depthTexture,xUv).r; float nxDepth=texture(depthTexture,nxUv).r; float yDepth=texture(depthTexture,yUv).r; float nyDepth=texture(depthTexture,nyUv).r; if(xDepth>=0.9999||nxDepth>=0.9999||yDepth>=0.9999||nyDepth>=0.9999) return false; position=viewPosition(at,depth); vec3 px=viewPosition(xUv,xDepth); vec3 nx=viewPosition(nxUv,nxDepth); vec3 py=viewPosition(yUv,yDepth); vec3 ny=viewPosition(nyUv,nyDepth); vec3 candidate=cross(py-ny,px-nx); float candidateLengthSquared=dot(candidate,candidate); if(candidateLengthSquared<=0.0001) return false; normal=candidate*inversesqrt(candidateLengthSquared); if(dot(normal,-position)<0.0) normal=-normal; return true; }\n"
        "vec3 filteredScreenReflection(vec2 at, vec2 texel, float roughness){ vec2 radius=texel*(1.0+clamp(roughness,0.0,1.0)*3.0); vec3 color=texture(hdrTexture,at).rgb*0.40; color+=texture(hdrTexture,clamp(at+vec2(radius.x,0.0),vec2(0.002),vec2(0.998))).rgb*0.15; color+=texture(hdrTexture,clamp(at-vec2(radius.x,0.0),vec2(0.002),vec2(0.998))).rgb*0.15; color+=texture(hdrTexture,clamp(at+vec2(0.0,radius.y),vec2(0.002),vec2(0.998))).rgb*0.15; color+=texture(hdrTexture,clamp(at-vec2(0.0,radius.y),vec2(0.002),vec2(0.998))).rgb*0.15; return color; }\n"
        "vec3 screenReflection(vec2 at, vec2 texel, float depth, vec3 base){ vec3 position; vec3 normal; if(!screenReflectionSurface(at,texel,depth,position,normal)) return base; float materialRoughness=clamp(texture(roughnessTexture,at).r,0.045,1.0); if(materialRoughness>0.55) return base; vec3 direction=normalize(reflect(normalize(position),normal)); vec3 reflected=base; float confidence=0.0; float hitDistance=0.0; float maxDistance=clamp(ssrMaxDistance,0.25,32.0); for(int step=1;step<=16;++step){ float distanceAlongRay=maxDistance*(float(step)/16.0); vec3 rayPosition=position+direction*distanceAlongRay; vec4 clip=projection*vec4(rayPosition,1.0); if(clip.w<=0.0) break; vec2 sampleUv=clip.xy/clip.w*0.5+0.5; if(sampleUv.x<0.002||sampleUv.x>0.998||sampleUv.y<0.002||sampleUv.y>0.998) break; float sceneDepth=texture(depthTexture,sampleUv).r; if(sceneDepth>=0.9999) continue; vec3 scenePosition=viewPosition(sampleUv,sceneDepth); float depthDelta=abs(scenePosition.z-rayPosition.z); if(depthDelta<=max(ssrThickness,0.001)){ vec3 hitPosition; vec3 hitNormal; if(!screenReflectionSurface(sampleUv,texel,sceneDepth,hitPosition,hitNormal) || dot(hitNormal,-direction)<=0.05) continue; reflected=filteredScreenReflection(sampleUv,texel,materialRoughness); hitDistance=distanceAlongRay; float edgeDistance=min(min(sampleUv.x,1.0-sampleUv.x),min(sampleUv.y,1.0-sampleUv.y)); float edgeConfidence=clamp(edgeDistance/max(ssrEdgeFade,0.001),0.0,1.0); float roughnessConfidence=(1.0-materialRoughness)*(1.0-materialRoughness); confidence=0.45*edgeConfidence*roughnessConfidence*(1.0-clamp(hitDistance/maxDistance,0.0,1.0)*0.5); break; } } return mix(base,reflected,clamp(confidence,0.0,0.45)); }\n"
        "vec3 screenReflectionRefined(vec2 at, vec2 texel, float depth, vec3 base){ vec3 position; vec3 normal; if(!screenReflectionSurface(at,texel,depth,position,normal)) return base; float materialRoughness=clamp(texture(roughnessTexture,at).r,0.045,1.0); if(materialRoughness>0.55) return base; vec3 direction=normalize(reflect(normalize(position),normal)); vec3 reflected=base; float confidence=0.0; float hitDistance=0.0; float maxDistance=clamp(ssrMaxDistance,0.25,32.0); float previousDistance=0.0; bool previousValid=false; for(int step=1;step<=16;++step){ float distanceAlongRay=maxDistance*(float(step)/16.0); vec3 rayPosition=position+direction*distanceAlongRay; vec4 clip=projection*vec4(rayPosition,1.0); if(clip.w<=0.0) break; vec2 sampleUv=clip.xy/clip.w*0.5+0.5; if(sampleUv.x<0.002||sampleUv.x>0.998||sampleUv.y<0.002||sampleUv.y>0.998) break; float sceneDepth=texture(depthTexture,sampleUv).r; if(sceneDepth>=0.9999){ previousValid=false; continue; } vec3 scenePosition=viewPosition(sampleUv,sceneDepth); float thickness=max(ssrThickness,0.001); float depthDelta=scenePosition.z-rayPosition.z; if(depthDelta>=0.0 && depthDelta<=thickness){ vec2 bestUv=sampleUv; float bestDelta=depthDelta; float bestDistance=distanceAlongRay; float low=previousValid?previousDistance:max(0.0,distanceAlongRay-maxDistance/16.0); float high=distanceAlongRay; for(int refine=0;refine<4;++refine){ float refinedDistance=(low+high)*0.5; vec3 refinedPosition=position+direction*refinedDistance; vec4 refinedClip=projection*vec4(refinedPosition,1.0); if(refinedClip.w<=0.0){ low=refinedDistance; continue; } vec2 refinedUv=refinedClip.xy/refinedClip.w*0.5+0.5; if(refinedUv.x<0.002||refinedUv.x>0.998||refinedUv.y<0.002||refinedUv.y>0.998){ low=refinedDistance; continue; } float refinedDepth=texture(depthTexture,refinedUv).r; if(refinedDepth>=0.9999){ low=refinedDistance; continue; } float refinedDelta=abs(viewPosition(refinedUv,refinedDepth).z-refinedPosition.z); if(refinedDelta<bestDelta){ bestDelta=refinedDelta; bestUv=refinedUv; bestDistance=refinedDistance; } if(refinedDelta<=thickness) high=refinedDistance; else low=refinedDistance; } float bestDepth=texture(depthTexture,bestUv).r; vec3 hitPosition; vec3 hitNormal; if(!screenReflectionSurface(bestUv,texel,bestDepth,hitPosition,hitNormal) || dot(hitNormal,-direction)<=0.05){ previousValid=false; continue; } reflected=filteredScreenReflection(bestUv,texel,materialRoughness); hitDistance=bestDistance; float edgeDistance=min(min(bestUv.x,1.0-bestUv.x),min(bestUv.y,1.0-bestUv.y)); float edgeConfidence=clamp(edgeDistance/max(ssrEdgeFade,0.001),0.0,1.0); float roughnessConfidence=(1.0-materialRoughness)*(1.0-materialRoughness); float fresnelConfidence=schlickFresnel(max(dot(normal,normalize(-position)),0.0)); confidence=0.45*edgeConfidence*roughnessConfidence*fresnelConfidence*(1.0-clamp(hitDistance/maxDistance,0.0,1.0)*0.5); break; } previousDistance=distanceAlongRay; previousValid=depthDelta<0.0; } return mix(base,reflected,clamp(confidence,0.0,0.45)); }\n"
        "#define screenReflection screenReflectionRefined\n"
    ...
    static const char* ibl_irradiance_fragment =
        ...
        "vec3 cosineSample(vec2 xi,vec3 n){ float phi=2.0*PI*xi.y; float sinTheta=sqrt(xi.x); float cosTheta=sqrt(max(1.0-xi.x,0.0)); vec3 up=abs(n.y)<0.95?vec3(0.0,1.0,0.0):vec3(1.0,0.0,0.0); vec3 t=normalize(cross(up,n)); vec3 b=cross(n,t); return normalize(t*cos(phi)*sinTheta+b*sin(phi)*sinTheta+n*cosTheta); }\n"
    ...
    const henka_scene* scene)
{
    ...
    henka_set_uniform_float_owned(
        ...
        &state->environment_shader_data,
        "sunIntensity",
        scene->environment.sun.enabled ? scene->environment.sun.intensity : 0.0f);
    henka_set_uniform_vec4_owned(
        state->environment_program,
        &state->environment_shader_data,
        "atmosphereParameters",
        (henka_vec4){
            scene->environment.atmosphere.rayleigh_scattering,
            scene->environment.atmosphere.mie_scattering,
            scene->environment.atmosphere.density,
            scene->environment.atmosphere.turbidity});
    henka_set_uniform_vec3_owned(
        state->environment_program,
        &state->environment_shader_data,
        "atmosphereGroundAlbedo",
        scene->environment.atmosphere.ground_albedo);
    henka_set_uniform_float_owned(
        state->environment_program,
        &state->environment_shader_data,
        "horizonIntensity",
        scene->environment.atmosphere.horizon_intensity);
    henka_set_uniform_float_owned(
        state->environment_program,
        &state->environment_shader_data,
        "sunAngularRadius",
    ...
    bool use_rendered_post_processing)
{
    ...
    henka_set_uniform_int_owned(state->tone_program, &state->tone_shader_data, "roughnessTexture", 7);
    ...
    henka_set_uniform_float_owned(state->tone_program, &state->tone_shader_data, "ssgiMaxRadiance", 4.0f);
...
static bool henka_opengl_allocate_reflection_probe_cube(
    GLuint* out_texture,
    int levels)
{
    GLuint texture = 0U;
    ...
    if (out_texture == NULL || levels <= 0 ||
        levels > HENKA_REFLECTION_PROBE_PREFILTER_LEVELS)
    {
        return false;
    }
    *out_texture = 0U;
    ...
    GLuint destination_texture)
{
    ...
    if (state == NULL || source_texture == 0U || destination_texture == 0U ||
    ...
    g_gl.FramebufferRenderbuffer(
        ...
        0U);
    g_gl.ActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_CUBE_MAP, source_texture);
    glDrawBuffer(GL_COLOR_ATTACHMENT0);
    glReadBuffer(GL_COLOR_ATTACHMENT0);
    for (face = 0; face < 6; ++face)
    {
        ...
        if (g_gl.CheckFramebufferStatus(GL_FRAMEBUFFER) != GL_FRAMEBUFFER_COMPLETE)
        {
            goto prefilter_restore;
        }
        glBindTexture(GL_TEXTURE_CUBE_MAP, destination_texture);
        glCopyTexSubImage2D(
            GL_TEXTURE_CUBE_MAP_POSITIVE_X + face,
            0,
            0,
    ...
    const henka_scene* scene)
{
    ...
    GLboolean previous_scissor_enabled = GL_FALSE;
    GLboolean previous_depth_enabled = GL_TRUE;
    ...
    if (!henka_opengl_ensure_reflection_probe_target(state) ||
        !henka_opengl_allocate_reflection_probe_cube(
            &candidate,
            HENKA_REFLECTION_PROBE_PREFILTER_LEVELS) ||
        !henka_opengl_allocate_reflection_probe_cube(&source, 1))
    {
        if (candidate != 0U)
        {
    ...
    {
        while (glGetError() != GL_NO_ERROR) {}
        success = henka_opengl_prefilter_reflection_probe(state, source, candidate);
    }
    if (source != 0U)
    {
        glDeleteTextures(1, &source);
    ...
    const struct henka_scene* scene)
{
    ...
    for (pass = 0U; pass < pass_count; ++pass)
    {
        ...
        henka_vec4 environment_atmosphere;
...
#define henka_set_uniform_float(program_value, name_value, value_value) \
    henka_set_uniform_float_owned(program_value, shader_data, name_value, value_value)
        ...
        henka_set_uniform_int(program, "environmentMode", (int)scene->environment.mode);
        henka_set_uniform_vec3(program, "environmentSunDirection", scene->environment.sun.direction);
        henka_set_uniform_vec3(program, "environmentSunColor", scene->environment.sun.color);
        henka_set_uniform_float(
            program,
            "environmentSunIntensity",
            scene->environment.sun.enabled ? scene->environment.sun.intensity : 0.0f);
        environment_atmosphere = (henka_vec4){
            scene->environment.atmosphere.rayleigh_scattering,
            scene->environment.atmosphere.mie_scattering,
            scene->environment.atmosphere.density,
            scene->environment.atmosphere.turbidity};
        ...
        henka_set_uniform_bool(program, "useThicknessTexture",
            thickness_texture_data != NULL && thickness_texture_data->texture_id != 0U);
...
#undef henka_set_uniform_float
        ...
        g_gl.ActiveTexture(GL_TEXTURE14);
        glBindTexture(GL_TEXTURE_2D,
