...
static void henka_memory_record_peak(size_t current_count)
{
    size_t peak_count = henka_memory_counter_load(&g_peak_allocation_count);

    while (current_count > peak_count &&
