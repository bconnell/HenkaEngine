    ...
    henka_authoring_modeling_report* out_report)
{
    ...
    modeling_report_reset(out_report);
    if (mesh == NULL || out_last_face_id == NULL || edge_offset > 1U || cut_count == 0U ||
        !henka_authoring_mesh_validate(mesh))
    {
        return HENKA_ERROR_INVALID_ARGUMENT;
    }
    if (!henka_checked_size_add(cut_count, 1U, &strip_count) ||
        !henka_checked_size_multiply(cut_count, 3U, &created_edges) ||
        !henka_checked_size_multiply(cut_count, 2U, &cut_pair_count) ||
        !henka_checked_size_multiply(cut_pair_count, sizeof(*cut_vertices), &cut_bytes) ||
        !henka_checked_size_multiply(cut_pair_count, sizeof(*cut_uvs), &cut_uv_bytes))
    {
        return HENKA_ERROR_LIMIT;
    }
    source_face = henka_authoring_mesh_get_face(mesh, face_id);
    ...
    {
        const henka_authoring_edge* edge = henka_authoring_mesh_get_edge(
            mesh, source_face->edges[index]);
        if (edge == NULL || edge->face_count != 1U || edge->faces[0] != face_id)
        {
            return HENKA_ERROR_INVALID_ARGUMENT;
        }
        source_vertices[index] = source_face->vertices[index];
        source_uvs[index] = source_face->uvs[index];
        if (henka_authoring_mesh_get_vertex(mesh, source_vertices[index]) == NULL)
        {
            return HENKA_ERROR_INVALID_ARGUMENT;
        }
    }
    before = henka_authoring_mesh_get_counts(mesh);
    {
        ...
        if (!henka_checked_size_multiply(cut_count, 2U, &required_vertices) ||
            ...
            desc.max_edges - before.edges < created_edges ||
            desc.max_faces - before.faces < cut_count)
        {
            return HENKA_ERROR_LIMIT;
        }
    }
    cut_vertices = (henka_authoring_vertex_id*)henka_malloc(cut_bytes);
    cut_uvs = (henka_vec2*)henka_malloc(cut_uv_bytes);
    if (cut_vertices == NULL || cut_uvs == NULL)
    {
        result = HENKA_ERROR_OUT_OF_MEMORY;
        goto cleanup;
    }
    ...
    for (index = 0U; index < cut_count; ++index)
    {
        const size_t first_corner = edge_offset;
        const size_t second_corner = (edge_offset + 1U) % 4U;
        const size_t opposite_corner = (edge_offset + 2U) % 4U;
        const size_t fourth_corner = (edge_offset + 3U) % 4U;
        const henka_authoring_vertex* first_vertex = henka_authoring_mesh_get_vertex(
            mesh, source_vertices[first_corner]);
        const henka_authoring_vertex* second_vertex = henka_authoring_mesh_get_vertex(
            mesh, source_vertices[second_corner]);
        const henka_authoring_vertex* opposite_vertex = henka_authoring_mesh_get_vertex(
            mesh, source_vertices[opposite_corner]);
        const henka_authoring_vertex* fourth_vertex = henka_authoring_mesh_get_vertex(
            mesh, source_vertices[fourth_corner]);
        const float factor = (float)(index + 1U) / (float)strip_count;
        const float opposite_factor = 1.0f - factor;
        const henka_vec3 first_delta = henka_vec3_subtract(
            second_vertex->position, first_vertex->position);
        const henka_vec3 opposite_delta = henka_vec3_subtract(
            fourth_vertex->position, opposite_vertex->position);
        const float first_length = henka_vec3_length(first_delta);
        const float opposite_length = henka_vec3_length(opposite_delta);
        const size_t first_cut = index * 2U;
        const size_t opposite_cut = first_cut + 1U;
        if (!isfinite(factor) || factor <= 0.0f || factor >= 1.0f ||
            !isfinite(opposite_factor) || opposite_factor <= 0.0f ||
            opposite_factor >= 1.0f ||
            !isfinite(first_length) || first_length <= 1.0e-7f ||
            !isfinite(opposite_length) || opposite_length <= 1.0e-7f)
        {
            result = HENKA_ERROR_INVALID_ARGUMENT;
            goto cleanup;
        }
        cut_uvs[first_cut] = (henka_vec2){
            source_uvs[first_corner].x +
                (source_uvs[second_corner].x - source_uvs[first_corner].x) * factor,
            source_uvs[first_corner].y +
                (source_uvs[second_corner].y - source_uvs[first_corner].y) * factor};
        cut_uvs[opposite_cut] = (henka_vec2){
            source_uvs[opposite_corner].x +
                (source_uvs[fourth_corner].x - source_uvs[opposite_corner].x) * opposite_factor,
            source_uvs[opposite_corner].y +
                (source_uvs[fourth_corner].y - source_uvs[opposite_corner].y) * opposite_factor};
        if (!isfinite(cut_uvs[first_cut].x) || !isfinite(cut_uvs[first_cut].y) ||
            !isfinite(cut_uvs[opposite_cut].x) || !isfinite(cut_uvs[opposite_cut].y))
        {
            result = HENKA_ERROR_INVALID_ARGUMENT;
            goto cleanup;
        }
        result = henka_authoring_mesh_add_vertex(
            candidate,
            henka_vec3_add(first_vertex->position, henka_vec3_scale(first_delta, factor)),
            cut_uvs[first_cut], first_vertex->material_region, &cut_vertices[first_cut]);
        if (result != HENKA_SUCCESS)
        {
            goto cleanup;
        }
        result = henka_authoring_mesh_add_vertex(
            candidate,
            henka_vec3_add(
                opposite_vertex->position,
                henka_vec3_scale(opposite_delta, opposite_factor)),
            cut_uvs[opposite_cut], opposite_vertex->material_region,
            &cut_vertices[opposite_cut]);
        if (result != HENKA_SUCCESS)
        {
            goto cleanup;
        }
    }
    {
        ...
        const size_t opposite_corner = (edge_offset + 2U) % 4U;
        const size_t fourth_corner = (edge_offset + 3U) % 4U;
        vertices[0] = source_vertices[first_corner];
        vertices[1] = cut_vertices[0];
        vertices[2] = cut_vertices[1];
        vertices[3] = source_vertices[fourth_corner];
        uvs[0] = source_uvs[first_corner];
        uvs[1] = cut_uvs[0];
        uvs[2] = cut_uvs[1];
        uvs[3] = source_uvs[fourth_corner];
        update.face_id = source_face->id;
        update.vertices = vertices;
        update.uvs = uvs;
        update.corner_count = 4U;
        ...
        for (index = 1U; index <= cut_count; ++index)
        {
            henka_authoring_vertex_id new_face_vertices[4];
            henka_vec2 new_face_uvs[4];
            size_t new_face_corner = 0U;
            if (index == cut_count)
            {
                new_face_vertices[0] = cut_vertices[(index - 1U) * 2U];
                new_face_vertices[1] = source_vertices[second_corner];
                new_face_vertices[2] = source_vertices[opposite_corner];
                new_face_vertices[3] = cut_vertices[(index - 1U) * 2U + 1U];
                new_face_uvs[0] = cut_uvs[(index - 1U) * 2U];
                new_face_uvs[1] = source_uvs[second_corner];
                new_face_uvs[2] = source_uvs[opposite_corner];
                new_face_uvs[3] = cut_uvs[(index - 1U) * 2U + 1U];
            }
            else
            {
                new_face_vertices[0] = cut_vertices[(index - 1U) * 2U];
                new_face_vertices[1] = cut_vertices[index * 2U];
                new_face_vertices[2] = cut_vertices[index * 2U + 1U];
                new_face_vertices[3] = cut_vertices[(index - 1U) * 2U + 1U];
                new_face_uvs[0] = cut_uvs[(index - 1U) * 2U];
                new_face_uvs[1] = cut_uvs[index * 2U];
                new_face_uvs[2] = cut_uvs[index * 2U + 1U];
                new_face_uvs[3] = cut_uvs[(index - 1U) * 2U + 1U];
            }
            result = henka_authoring_mesh_add_face(
                candidate, new_face_vertices, 4U,
                source_face->material_region, source_face->smooth, &last_face_id);
            if (result != HENKA_SUCCESS)
            {
                goto cleanup;
            }
            for (new_face_corner = 0U; new_face_corner < 4U; ++new_face_corner)
            {
                result = henka_authoring_mesh_set_face_corner_uv(
                    candidate, last_face_id, new_face_corner, new_face_uvs[new_face_corner]);
                if (result != HENKA_SUCCESS)
                {
                    goto cleanup;
                }
    ...
    henka_authoring_modeling_report* out_report)
{
    ...
    for (cut_index = 0U; cut_index < cut_count; ++cut_index)
    {
        ...
        for (index = 0U; index < path_vertex_count; ++index)
        {
            ...
            first_vertex = henka_authoring_mesh_get_vertex(mesh, path_edge->vertices[0]);
            second_vertex = henka_authoring_mesh_get_vertex(mesh, path_edge->vertices[1]);
            if (first_vertex == NULL || second_vertex == NULL)
            {
                result = HENKA_ERROR_INVALID_ARGUMENT;
                goto cleanup;
            ...
            uv = (henka_vec2){
                face->uvs[corner].x +
                    (face->uvs[(corner + 1U) % face->corner_count].x - face->uvs[corner].x) * factor,
                face->uvs[corner].y +
                    (face->uvs[(corner + 1U) % face->corner_count].y - face->uvs[corner].y) * factor};
...
static bool modeling_bevel_selected_edge_contains(
    const henka_authoring_vertex_id* selected_edge_endpoints,
    size_t selected_edge_count,
    henka_authoring_vertex_id first_vertex_id,
    henka_authoring_vertex_id second_vertex_id)
{
    size_t index;
    if (selected_edge_endpoints == NULL || selected_edge_count == 0U)
    {
        return false;
    }
    for (index = 0U; index < selected_edge_count; ++index)
    {
        const henka_authoring_vertex_id selected_first =
            selected_edge_endpoints[index * 2U];
        const henka_authoring_vertex_id selected_second =
            selected_edge_endpoints[index * 2U + 1U];
        if ((selected_first == first_vertex_id && selected_second == second_vertex_id) ||
            (selected_first == second_vertex_id && selected_second == first_vertex_id))
        {
            return true;
        }
    }
...
static bool modeling_bevel_processed_face_contains(
    const henka_authoring_face_id* processed_face_ids,
    size_t processed_face_count,
    henka_authoring_face_id face_id)
{
    size_t index;
    if (processed_face_ids == NULL || processed_face_count == 0U)
    {
        return false;
    }
    for (index = 0U; index < processed_face_count; ++index)
    {
        if (processed_face_ids[index] == face_id)
        {
            return true;
        }
    }
    ...
    henka_authoring_modeling_report* out_report)
{
    ...
    for (face_index = 0U; face_index < 2U; ++face_index)
    {
        for (corner = 0U; corner < 4U; ++corner)
        {
            ...
            const bool adjacent_is_selected = adjacent != NULL &&
                modeling_bevel_selected_edge_contains(
                    selected_edge_endpoints, selected_edge_count,
                    adjacent->vertices[0], adjacent->vertices[1]);
            const bool adjacent_is_processed = adjacent != NULL &&
                ((adjacent->face_count > 0U && modeling_bevel_processed_face_contains(
                    processed_face_ids, processed_face_count, adjacent->faces[0])) ||
                 (adjacent->face_count > 1U && modeling_bevel_processed_face_contains(
                    processed_face_ids, processed_face_count, adjacent->faces[1])));
            if (adjacent == NULL || (corner != edge_corners[face_index] &&
                (adjacent->face_count != 1U || adjacent->faces[0] != faces[face_index]->id) &&
                !adjacent_is_selected && !adjacent_is_processed))
            {
                return HENKA_ERROR_INVALID_ARGUMENT;
            }
        }
    ...
    henka_authoring_modeling_report* out_report)
{
    ...
    henka_authoring_face_id* processed_face_ids = NULL;
    henka_authoring_mesh* candidate = NULL;
    henka_authoring_mesh_counts before;
    henka_authoring_mesh_counts after;
    henka_authoring_mesh_desc desc;
    ...
    if (edge_count > desc.max_edges ||
        ...
        !henka_checked_size_multiply(
            edge_count, sizeof(*processed_face_ids), &processed_face_bytes))
    {
        return HENKA_ERROR_LIMIT;
    }
    before = henka_authoring_mesh_get_counts(mesh);
    ...
    processed_face_ids = (henka_authoring_face_id*)henka_malloc(processed_face_bytes);
    if (sorted_edges == NULL || edge_endpoints == NULL || processed_face_ids == NULL)
    {
        result = HENKA_ERROR_OUT_OF_MEMORY;
        goto cleanup;
    }
    ...
    for (index = 0U; index < edge_count; ++index)
    {
        ...
        if (edge == NULL || (edge->face_count != 1U && edge->face_count != 2U) ||
            edge->vertices[0] == edge->vertices[1] ||
        ...
        {
            all_interior = false;
            if (modeling_vertex_incident_face_count(mesh, edge->vertices[0]) != 1U ||
    ...
    if (all_interior)
    {
        for (index = 0U; index < edge_count; ++index)
        {
            ...
            for (other_index = 0U; other_index < index; ++other_index)
            {
                ...
                const size_t shared_face_count = edge != NULL && other != NULL
                    ...
                    : SIZE_MAX;
                if (edge == NULL || other == NULL || shared_face_count > 1U ||
    ...
    if (all_interior)
    {
        if (required_vertices > SIZE_MAX - 2U ||
            desc.max_vertices < before.vertices ||
            desc.max_vertices - before.vertices < required_vertices + 2U)
        {
            result = HENKA_ERROR_LIMIT;
            goto cleanup;
        }
    ...
    if (all_interior)
    {
        size_t processed_face_count = 0U;
        for (index = 0U; index < edge_count; ++index)
        {
            ...
            result = modeling_bevel_interior_edge(
                candidate, current_edge_id, width,
                edge_endpoints, edge_count, processed_face_ids,
                processed_face_count, &step_report);
            if (result != HENKA_SUCCESS)
            {
                goto cleanup;
            }
            ...
            if (step_report.primary_face_id != HENKA_AUTHORING_INVALID_ID &&
                processed_face_count < edge_count)
            {
                processed_face_ids[processed_face_count++] = step_report.primary_face_id;
            }
        }
    }
    else if (all_same_face)
    ...
    henka_authoring_modeling_report* out_report)
{
    ...
    if (start_corner == SIZE_MAX || end_corner == SIZE_MAX)
    {
        return HENKA_ERROR_INVALID_ARGUMENT;
    }
    ...
    if (desc.max_vertices < 2U || desc.max_edges < 3U || desc.max_faces == 0U ||
        before.vertices > desc.max_vertices - 2U ||
        before.edges > desc.max_edges - 3U || before.faces >= desc.max_faces)
    {
        return HENKA_ERROR_LIMIT;
    }
    result = henka_authoring_mesh_clone(mesh, &candidate);
    ...
    {
        const size_t source_corner = corner == 0U ? start_corner : end_corner;
        const henka_authoring_vertex* vertex = henka_authoring_mesh_get_vertex(
            mesh, source_vertices[source_corner]);
        henka_vec3 position;
        if (vertex == NULL)
        {
            result = HENKA_ERROR_INVALID_ARGUMENT;
        ...
        position = henka_vec3_add(vertex->position, offset);
        if (!modeling_finite_vec3(position))
        {
            result = HENKA_ERROR_LIMIT;
            goto cleanup;
        }
        result = henka_authoring_mesh_add_vertex(
            candidate, position, source_uvs[source_corner], vertex->material_region,
            &new_vertices[corner]);
        if (result != HENKA_SUCCESS)
        {
            goto cleanup;
    ...
    {
        const henka_authoring_vertex_id side_vertices[4] = {
            source_vertices[start_corner], source_vertices[end_corner],
            new_vertices[1], new_vertices[0]};
        const henka_vec2 side_uvs[4] = {
            source_uvs[start_corner], source_uvs[end_corner],
            source_uvs[end_corner], source_uvs[start_corner]};
        size_t side_corner;
        result = henka_authoring_mesh_add_face(
            candidate, side_vertices, 4U, source_material_region, false, &new_face_id);
        if (result != HENKA_SUCCESS)
        {
            goto cleanup;
        }
        ...
        {
            result = henka_authoring_mesh_set_face_corner_uv(
                candidate, new_face_id, side_corner, side_uvs[side_corner]);
            if (result != HENKA_SUCCESS)
            {
                goto cleanup;
            }
    ...
    memcpy(updated_vertices, source_vertices,
        source_corner_count * sizeof(*updated_vertices));
    memcpy(updated_uvs, source_uvs, source_corner_count * sizeof(*updated_uvs));
    updated_vertices[start_corner] = new_vertices[0];
    updated_vertices[end_corner] = new_vertices[1];
    updated_uvs[start_corner] = source_uvs[start_corner];
    updated_uvs[end_corner] = source_uvs[end_corner];
    update.face_id = source_face_id;
    update.vertices = updated_vertices;
    update.uvs = updated_uvs;
    update.corner_count = source_corner_count;
    update.material_region = source_material_region;
    update.smooth = source_smooth;
    result = henka_authoring_mesh_apply_face_loop_updates_internal(candidate, &update, 1U);
    if (result != HENKA_SUCCESS)
    ...
    new_edge_id = modeling_find_edge_between_vertices(
        candidate, new_vertices[0], new_vertices[1]);
    if (new_edge_id == HENKA_AUTHORING_INVALID_ID ||
        henka_authoring_mesh_set_edge_hard(candidate, new_edge_id, source_edge_hard) != HENKA_SUCCESS ||
        !henka_authoring_mesh_validate(candidate) ||
        !modeling_face_geometry_is_valid(candidate))
    {
        result = HENKA_ERROR_INVALID_ARGUMENT;
    ...
    henka_authoring_modeling_report* out_report)
{
    ...
    if (edge_count > desc.max_edges ||
        ...
        !henka_checked_size_multiply(endpoint_count, sizeof(*edge_endpoints), &endpoint_bytes) ||
        !henka_checked_size_multiply(edge_count, 2U, &required_vertices) ||
        !henka_checked_size_multiply(edge_count, 3U, &required_edges) ||
    ...
    for (index = 0U; index < edge_count; ++index)
    {
        const henka_authoring_edge* edge = henka_authoring_mesh_get_edge(
            mesh, sorted_edges[index]);
        const henka_authoring_vertex_id first_vertex = edge == NULL
            ? HENKA_AUTHORING_INVALID_ID : edge->vertices[0];
        const henka_authoring_vertex_id second_vertex = edge == NULL
            ? HENKA_AUTHORING_INVALID_ID : edge->vertices[1];
        size_t previous;
        if (edge == NULL || edge->face_count != 1U || first_vertex == second_vertex ||
            henka_authoring_mesh_get_vertex(mesh, first_vertex) == NULL ||
            henka_authoring_mesh_get_vertex(mesh, second_vertex) == NULL)
        {
            result = HENKA_ERROR_INVALID_ARGUMENT;
            goto cleanup;
        }
        edge_endpoints[index * 2U] = first_vertex;
        edge_endpoints[index * 2U + 1U] = second_vertex;
        for (previous = 0U; previous < index; ++previous)
        {
            const henka_authoring_edge* other = henka_authoring_mesh_get_edge(
                mesh, sorted_edges[previous]);
            if (other == NULL || other->faces[0] == edge->faces[0] ||
                other->vertices[0] == first_vertex || other->vertices[0] == second_vertex ||
                other->vertices[1] == first_vertex || other->vertices[1] == second_vertex)
            {
                result = HENKA_ERROR_INVALID_ARGUMENT;
                goto cleanup;
            }
    ...
    {
        const henka_authoring_edge_id current_edge_id = modeling_find_edge_between_vertices(
            candidate, edge_endpoints[index * 2U], edge_endpoints[index * 2U + 1U]);
        henka_authoring_edge_id new_edge_id = HENKA_AUTHORING_INVALID_ID;
        henka_authoring_face_id new_face_id = HENKA_AUTHORING_INVALID_ID;
        if (current_edge_id == HENKA_AUTHORING_INVALID_ID)
        ...
        result = henka_authoring_mesh_extrude_edge(
            candidate, current_edge_id, distance, &new_edge_id, &new_face_id, &last_report);
    }
    if (result == HENKA_SUCCESS &&
        (!henka_authoring_mesh_validate(candidate) || !modeling_face_geometry_is_valid(candidate)))
    {
    ...
    if (result == HENKA_SUCCESS)
    {
        ...
        if (result == HENKA_SUCCESS)
        {
            ...
            if (out_report != NULL)
            {
                out_report->primary_edge_id = last_report.primary_edge_id;
                out_report->primary_face_id = last_report.primary_face_id;
            }
        }
    }
