...
foreach ($mode in @("Material Preview", "Rendered")) {
    ...
    if ($measurement.MeanSaturation -lt 5.0) {
        throw "$mode Terrain close evidence does not communicate material identity (mean saturation=$([Math]::Round($measurement.MeanSaturation, 2)))."
    }
    if ($measurement.QuadrantLumaRange -lt 1.0) {
        throw "$mode Terrain close evidence does not show spatially varied scene content (quadrant luma range=$([Math]::Round($measurement.QuadrantLumaRange, 2)))."
    }
}
