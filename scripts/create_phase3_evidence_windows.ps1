...
Write-Report -RelativePath "Handoff\remaining-campaign-tracks.txt" -Text "Not silently claimed complete: local probe editor visualization and automatic invalidation on arbitrary scene-entity changes, production instancing/texture residency streaming, AO/SSR beyond the documented IBL fallback, soak/fault coverage, CI verification, and same-camera app-only Solid/Material Preview/Rendered visual evidence.`r`n"
