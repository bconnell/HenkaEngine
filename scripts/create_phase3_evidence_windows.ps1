...
Write-Report -RelativePath "Screenshots\SCREENSHOT_INDEX.txt" -Text "No app-only screenshots were produced by this automated run. Manual capture remains an explicit gate.`r`n"
    Write-Report -RelativePath "Handoff\remaining-capabilities.txt" -Text "Remaining capability scope not covered by this evidence run: full production glTF scene coverage, KTX2/Basis GPU-native compressed upload, production instancing/texture residency streaming, planar/hierarchical/off-screen SSR coverage, transmission/glass materials, soak/fault coverage, and final closure audits.`r`n"
