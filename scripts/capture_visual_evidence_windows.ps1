...
function Assert-HenkaSubsurfaceReferenceCaptureMetadata {
    param(
        [Parameter(Mandatory = $true)][string]$Line,
        [Parameter(Mandatory = $true)][string]$Label,
        [Parameter(Mandatory = $true)][string]$ExpectedView,
        [Parameter(Mandatory = $true)][string]$ExpectedVariant
    )

    $pattern = '^\s*CAPTURE_READY_SSS_REFERENCE mode=(?<mode>[a-z_]+) view=(?<view>wide|close) reference_layout=(?<layout>[a-z_]+) reference_texture_edge=(?<texture_edge>\d+) reference_exposure_stops=(?<exposure>[-0-9.]+) reference_sss_variant=(?<variant>opaque|thin|thick) viewport=(?<vx>-?\d+),(?<vy>-?\d+),(?<vw>\d+),(?<vh>-?\d+) aspect=(?<aspect>[-0-9.]+) camera_position=(?<px>[-0-9.]+),(?<py>[-0-9.]+),(?<pz>[-0-9.]+) yaw=(?<yaw>[-0-9.]+) pitch=(?<pitch>[-0-9.]+) roll=(?<roll>[-0-9.]+) fov=(?<fov>[-0-9.]+) reference_bounds=(?<cx>[-0-9.]+),(?<cy>[-0-9.]+),(?<cz>[-0-9.]+),(?<ex>[-0-9.]+),(?<ey>[-0-9.]+),(?<ez>[-0-9.]+) reference_midpoint=(?<mx>[-0-9.]+),(?<my>[-0-9.]+) reference_count=(?<count>\d+) settled_frames=(?<sf>\d+) draw_expected=1\s*$'
    $match = [regex]::Match($Line, $pattern)
    if (-not $match.Success) {
        throw "Subsurface reference capture readiness metadata was malformed for $Label."
    }
    $expectedLayout = if ($ExpectedView -eq "close") { "close_grid" } else { "wide_row" }
    if ($match.Groups["view"].Value -ne $ExpectedView -or
        $match.Groups["variant"].Value -ne $ExpectedVariant -or
        $match.Groups["layout"].Value -ne $expectedLayout -or
        [int]$match.Groups["texture_edge"].Value -lt 32 -or
        [int]$match.Groups["count"].Value -ne 9 -or
        [int]$match.Groups["sf"].Value -lt 3) {
        throw "Subsurface reference capture readiness metadata is incomplete for $Label."
    }
    return [pscustomobject]@{
        Canonical = ($Line -replace 'mode=[^ ]+', 'mode=shared' -replace 'reference_sss_variant=[^ ]+', 'reference_sss_variant=varied')
        Mode = $match.Groups["mode"].Value
        View = $match.Groups["view"].Value
        Layout = $match.Groups["layout"].Value
...
function Assert-HenkaColorSpaceReferenceCaptureMetadata {
    param(
        [Parameter(Mandatory = $true)][string]$Line,
        [Parameter(Mandatory = $true)][string]$Label,
        [Parameter(Mandatory = $true)][string]$ExpectedView
    ...
    $pattern = '^\s*CAPTURE_READY_COLOR_SPACE_REFERENCE mode=(?<mode>solid|material_preview|rendered) view=(?<view>wide|close) reference_layout=(?<layout>[a-z_]+) reference_texture_edge=(?<texture_edge>\d+) .*color_space_reference=1 color_space_srgb=1 color_space_linear=1 color_space_shared_source=1 color_space_srgb_count=4 color_space_linear_count=5 viewport=(?<vx>-?\d+),(?<vy>-?\d+),(?<vw>\d+),(?<vh>-?\d+) .*reference_count=(?<count>\d+) settled_frames=(?<settled>\d+) draw_expected=1\s*$'
    $match = [regex]::Match($Line, $pattern)
    if (-not $match.Success -or
        $match.Groups["view"].Value -ne $ExpectedView -or
        $match.Groups["layout"].Value -ne $(if ($ExpectedView -eq "close") { "close_grid" } else { "wide_row" }) -or
        ...
        [int]$match.Groups["count"].Value -ne 9 -or
        [int]$match.Groups["settled"].Value -lt 3) {
        throw "PBR color-space reference capture readiness metadata was malformed for $Label."
    }
    return [pscustomobject]@{
        Canonical = ($Line -replace 'mode=[^ ]+', 'mode=shared')
        Mode = $match.Groups["mode"].Value
...
function Assert-HenkaEnergyReferenceCaptureMetadata {
    param(
        [Parameter(Mandatory = $true)][string]$Line,
        [Parameter(Mandatory = $true)][string]$Label,
        [Parameter(Mandatory = $true)][string]$ExpectedView
    ...
    $pattern = '^\s*CAPTURE_READY_ENERGY_REFERENCE mode=(?<mode>solid|material_preview|rendered) view=(?<view>wide|close) reference_layout=(?<layout>[a-z_]+) reference_texture_edge=(?<texture_edge>\d+) .*energy_reference=1 energy_dielectric_count=3 energy_metallic_count=3 energy_transmission_count=3 energy_clipped_channel_limit=254 viewport=(?<vx>-?\d+),(?<vy>-?\d+),(?<vw>\d+),(?<vh>-?\d+) .*reference_count=(?<count>\d+) settled_frames=(?<settled>\d+) draw_expected=1\s*$'
    $match = [regex]::Match($Line, $pattern)
    if (-not $match.Success -or
        $match.Groups["view"].Value -ne $ExpectedView -or
        $match.Groups["layout"].Value -ne $(if ($ExpectedView -eq "close") { "close_grid" } else { "wide_row" }) -or
        ...
        [int]$match.Groups["count"].Value -ne 9 -or
        [int]$match.Groups["settled"].Value -lt 3) {
        throw "PBR energy reference capture readiness metadata was malformed for $Label."
    }
    return [pscustomobject]@{
        Canonical = ($Line -replace 'mode=[^ ]+', 'mode=shared')
        Mode = $match.Groups["mode"].Value
...
function Assert-HenkaIblReferenceCaptureMetadata {
    ...
    $pattern = '^\s*CAPTURE_READY_IBL_REFERENCE mode=rendered view=(?<view>wide|close) reference_layout=(?<layout>[a-z_]+) reference_texture_edge=(?<texture_edge>\d+) .*ibl_reference=1 (?:ibl_diagnostic=(?<diagnostic>[a-z_]+) )?(?:ibl_control=(?<control>[a-z_]+) ibl_rotation_degrees=(?<rotation>-?[0-9.]+) ibl_prefilter_lod_override=(?<override>-?[0-9.]+) )?ibl_direct_lighting=0 ibl_moon_lighting=0 ibl_roughness_ladder=1 ibl_roughness_samples=9 ibl_irradiance_resolution=32 ibl_prefilter_resolution=256 ibl_prefilter_levels=7 ibl_brdf_resolution=128 .*viewport=(?<vx>-?\d+),(?<vy>-?\d+),(?<vw>\d+),(?<vh>-?\d+) .*reference_count=(?<count>\d+) settled_frames=(?<settled>\d+) draw_expected=1\s*$'
    $match = [regex]::Match($Line, $pattern)
    if (-not $match.Success -or
        $match.Groups["view"].Value -ne $ExpectedView -or
        $match.Groups["layout"].Value -ne $(if ($ExpectedView -eq "close") { "close_grid" } else { "wide_row" }) -or
...
if ($EvidenceProfile -eq "PBR_COLOR_SPACE_REFERENCE") {
    $modes = @(
        @{ Label = "color_space_reference_solid"; Arguments = @("--capture-realism-reference", "color_space", $ReferenceView, "solid"); File = "color-space-reference-$ReferenceView-solid.png" },
        @{ Label = "color_space_reference_material_preview"; Arguments = @("--capture-realism-reference", "color_space", $ReferenceView, "material_preview"); File = "color-space-reference-$ReferenceView-material-preview.png" },
        @{ Label = "color_space_reference_rendered"; Arguments = @("--capture-realism-reference", "color_space", $ReferenceView, "rendered"); File = "color-space-reference-$ReferenceView-rendered.png" }
    )
}
...
if ($EvidenceProfile -eq "PBR_IBL_FRACTIONAL") {
    $modes = @(
        @{ Label = "ibl_ordinary_lod_1_00"; Control = "ordinary_prefilter_lod"; Diagnostic = "none"; Rotation = 0.0; Override = 1.00; Arguments = @("--capture-realism-reference", "ibl_ordinary_mip", "1.00", $ReferenceView, "rendered"); File = "ibl-ordinary-lod-1.00-$ReferenceView-rendered.png" },
        @{ Label = "ibl_ordinary_lod_1_25"; Control = "ordinary_prefilter_lod"; Diagnostic = "none"; Rotation = 0.0; Override = 1.25; Arguments = @("--capture-realism-reference", "ibl_ordinary_mip", "1.25", $ReferenceView, "rendered"); File = "ibl-ordinary-lod-1.25-$ReferenceView-rendered.png" },
        @{ Label = "ibl_ordinary_lod_1_50"; Control = "ordinary_prefilter_lod"; Diagnostic = "none"; Rotation = 0.0; Override = 1.50; Arguments = @("--capture-realism-reference", "ibl_ordinary_mip", "1.50", $ReferenceView, "rendered"); File = "ibl-ordinary-lod-1.50-$ReferenceView-rendered.png" },
        @{ Label = "ibl_ordinary_lod_1_75"; Control = "ordinary_prefilter_lod"; Diagnostic = "none"; Rotation = 0.0; Override = 1.75; Arguments = @("--capture-realism-reference", "ibl_ordinary_mip", "1.75", $ReferenceView, "rendered"); File = "ibl-ordinary-lod-1.75-$ReferenceView-rendered.png" },
        @{ Label = "ibl_ordinary_lod_2_00"; Control = "ordinary_prefilter_lod"; Diagnostic = "none"; Rotation = 0.0; Override = 2.00; Arguments = @("--capture-realism-reference", "ibl_ordinary_mip", "2.00", $ReferenceView, "rendered"); File = "ibl-ordinary-lod-2.00-$ReferenceView-rendered.png" }
    )
}
...
if ($EvidenceProfile -eq "SUBSURFACE_REFERENCE") {
    $modes = @(
        @{ Label = "sss_reference_opaque"; Arguments = @("--capture-realism-reference", "sss", $ReferenceView, "opaque", "rendered"); File = "sss-reference-$ReferenceView-opaque.png"; Variant = "opaque" },
        @{ Label = "sss_reference_thin"; Arguments = @("--capture-realism-reference", "sss", $ReferenceView, "thin", "rendered"); File = "sss-reference-$ReferenceView-thin.png"; Variant = "thin" },
...
if ($EvidenceProfile -eq "PBR_COLOR_SPACE_REFERENCE") {
    $records.Add("PBR color-space reference: four sRGB and five linear matched subjects from one shared source; view=$ReferenceView")
}
if ($EvidenceProfile -eq "PBR_ENERGY_REFERENCE") {
...
if ($EvidenceProfile -eq "PBR_IBL_FRACTIONAL") {
    $records.Add("PBR IBL ordinary-path LOD isolation: same production branch at exact and fractional prefilter LOD values 1.00-2.00; view=$ReferenceView")
}
if ($EvidenceProfile -eq "LIGHTING_REFERENCE") {
...
foreach ($mode in $modes) {
    ...
    try {
        ...
        elseif ($EvidenceProfile -eq "PBR_COLOR_SPACE_REFERENCE") {
            [void]$captureMetadata.Add(
                (Assert-HenkaColorSpaceReferenceCaptureMetadata -Line $metadataLine -Label $mode.Label -ExpectedView $ReferenceView))
        }
        ...
        elseif ($EvidenceProfile -eq "PBR_IBL_FRACTIONAL") {
            [void]$captureMetadata.Add(
                (Assert-HenkaIblIsolationCaptureMetadata -Line $metadataLine -Label $mode.Label -ExpectedView $ReferenceView -ExpectedControl $mode.Control -ExpectedDiagnostic $mode.Diagnostic -ExpectedRotation $mode.Rotation -ExpectedOverride $mode.Override))
        }
...
if ($captureMetadata.Count -gt 1) {
    ...
    elseif ($EvidenceProfile -eq "PBR_IBL_FRACTIONAL") {
        $records.Add("Composition metadata: identical across ordinary production-path fractional prefilter LOD controls")
    }
    else {
...
if ($EvidenceProfile -eq "PBR_COLOR_SPACE_REFERENCE") {
    $records | Set-Content -LiteralPath (Join-Path $OutputDirectory "INDEX.txt")
    & (Join-Path $PSScriptRoot "check_pbr_color_space_reference_windows.ps1") `
        -InputDirectory $OutputDirectory
}
if ($EvidenceProfile -eq "PBR_ENERGY_REFERENCE") {
    $records | Set-Content -LiteralPath (Join-Path $OutputDirectory "INDEX.txt")
...
elseif ($EvidenceProfile -eq "PBR_IBL_FRACTIONAL") {
    "PBR IBL ordinary-path fractional LOD evidence"
}
elseif ($EvidenceProfile -eq "SCENE_PROBE_REFERENCE") {
