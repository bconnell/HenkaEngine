...
try {
    ...
    (Get-Content -LiteralPath (Join-Path $fixtureRoot "ssgi_performance_reference.stdout.txt") -Raw) -replace "reference_probe_diffuse_active=1 ", "reference_probe_diffuse_active=1 reference_probe_prefilter_active=1 " |
        Set-Content -LiteralPath (Join-Path $fixtureRoot "ssgi_performance_reference.stdout.txt")
