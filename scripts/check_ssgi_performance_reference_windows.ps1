...
if ($stdoutText -notmatch "(?m)CAPTURE_READY_SSGI_PERFORMANCE_REFERENCE .* reference_probe_prefilter_active=1 reference_probe_blend_active=1 reference_probe_enabled_count=\d+ reference_probe_captured_count=\d+ reference_probe_capture_generation=\d+ reference_probe_capture_failures=0 ") {
...
if (-not $probeHealth.Success -or
    ...
    throw 'SSGI performance metadata did not prove two current captured probes without failures.'
}
